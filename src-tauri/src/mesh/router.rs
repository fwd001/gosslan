...
impl MeshRouter {
    ...
    ///
    /// ⚠️ **生产路径不走这个函数**（架构复审 0-A2 明确登记，不是"待接线"）：
    /// 群消息与好友申请的洪泛在 `outbound.rs::broadcast_gossip`，那里**刻意只用
    /// `exclude_source`** —— 按 `fanout` 截断会把群成员静默切掉（少发 = 有人永久收不到，
    /// 比多转发一跳严重得多）。真正的"该发给谁"由信封里的受众名单决定。
    /// 本函数连同它的截断逻辑只被自身测试覆盖；要启用它，先回答"截断掉的成员怎么办"。
