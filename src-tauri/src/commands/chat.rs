...
// - 消息删除、合并转发
// ⚠️ **撤回与重发不在这里**（2026-09-25 删掉了 `recall_message` / `resend_message` /
// `cancel_send` —— 前端一次都没调过）：撤回只有群聊版（`group_announcements.rs`），
// 界面上的「重发」是 `chat.send` 重发一条**新消息**。
// 重做「按原 msg_id 重投」前必读 docs/protocol-invariants.md §6 那两条事故规则。
// ---------------- 单聊（Gossip + E2EE） ----------------
