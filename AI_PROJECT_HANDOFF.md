...
| 和自己聊天 | 会话列表「＋」里的入口。会话 id = **本机 device_id** 的本地会话（消息只落本机、不走网络、无回执）；可当备忘录。只支持文本（图片/文件明确报错）。见 §4.2 的第四条路径 |
| 群任务 | 群聊头部按钮 + 群成员面板分区的「群任务」面板：任何成员可建、指派 1+ 人；四态**手动选**（待办/进行中/延期/完成，不做截止时间）；被指派人改状态，创建者或群主改标题/指派人/删除 |
...
⚠️ **第四条路径：不出本机**（「和自己聊天」，`commands::insert_self_message`）。收发双方都是本机，
既没有链路可发、也没有对端可等 Ack —— 它只落本地库、**不进 outbox、不发 gossip、不加密**
（E2EE 保护的是传输；本地消息与其它消息一样是 SQLite 明文）。反过来说：**永远不要**把自聊消息
接回上面三条路径中的任何一条（进了 outbox 的那一行会永远等不到 Ack、被心跳无限重发）。
`lib.rs` 的 `self_chat_stays_local` 护栏钉着这条。
