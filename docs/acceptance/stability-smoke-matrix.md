...
`summary.json.shots` 记相对路径、`summary.html` 内嵌图集；判据**只钉「落盘且不是空图」**（实测 1.77 / 1.93 MB，阈值 200 KiB），
