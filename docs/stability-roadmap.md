...
| A-2 故障注入 | **E2E_VERIFIED（八格，全部接进 `npm run verify:e2e` 本地层）**：脏 `.part`→错 hash→补齐 / 真前缀必须被续传 / 接收中真 `SIGKILL` 后续完 / 对端失联后解冻自愈 / 接收目录写不进去 / 入队后源文件被改小 / 连续三单其中两单同名 / 字节收完只塌在最后那次改名。每格都有正向绿 + 只翻一个期望值的 `-lie` 反向红（**断言条数不在这里写**：由 `check-doc-numbers.mjs` 现算并与验收矩阵对账）。ADR-0010 已从 Proposed 落到「有八条真跑绿的注入」 | §八 剩余：**三实例并发（两个 offer 都在任一次 rename 之前到达）**。**尺寸阶梯已按「形状」而不是逐 MB 补进本地层**：`--size=0.001`（1 KB = 单片、零头即整份）与 `--size=10`（10 MB = 多片 + 最后一片是零头）各跑**整趟默认轮**（文本 + 文件 + 重启，16 条断言同源），两档实测全绿、10 MB 档整轮 11.2s ⇒ 10 KB 与 1 KB 同形状（LAN 片长 256 KiB）由 1 KB 档代表，**100 MB 及以上不进本地层**（一次几分钟会拖垮本地与 CI），那一档的大文件窗口由「杀进程轮 100 MB」代偿。⚠️ 曾并列在这条"剩余"里的另两格**已经不成立，别照抄**：**DB 锁竞争**升 L2（见本节「§十二 DB 与锁」行，两条动态判据 + 变异自证）、**连续多文件**由注入⑤覆盖（只覆盖"同 peer 串行 flush"这一时序）。**目标目录变化**按形状拆两半各自已做：目录仍在但不可写（`EACCES`）＝L5 注入⑧两个真实进程；整个目录被删走（接收句柄挂在被 unlink 的 inode 上、写入与 fsync 照旧成功 ⇒ **只剩最后一次 rename 能发现成品无处安放**）＝L2 `network/file.rs::rotted_receive_directory_finishes_failed_never_done`（已用"把 rename 失败吞掉就算成功"变异自证会报红）。⚠️ 重复帧·乱序帧·旧 attempt 帧·重复 FileDone 这四格**不在"未做"里**：L2 已有具名证据（见 §11 harness 边界行的分层清单），L5 帧级伪造判为**会造出假绿 ⇒ 不可自动**；「单链路断开」同样单机无 root 不可自动（伪造被 route 优先级绕过）|
...
它证明的是"今天的行为正确"，不是"修了一个缺陷"；§七里真正还剩的空格只有**三实例并发**那一格。L-C 只剩"两个发送者→一个接收者"的真并发与断链才有增量 ⇒ **延后** | 延后（要做就得连三实例一起重开） |
...
剩余：**三实例并发** ⚠️（唯一未做格，且**修法要用户拍板**，见 12.9）。
