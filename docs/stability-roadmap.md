...
| **A-12** `.part` 字节数冒充「已收完」 | **本轮收口（待最后一道全量门禁）**：按候选①改码 + 单测先红后绿 + 注入⑧两侧一致（`A=failed / B=failed`）+ 交叉自洽断言加回 + 不变量 §17 第三条挂上钩子 | 全量层绿后升 **GUARDS_LOCKED**；边界另记：目录恢复可写之后这一单**不会**自己补完（协议里没有那条码），不算这一格欠账 |
