...
| ★★ **2026-09-26 实测**：上面那格**试着补的时候撞出一件更严重的事** | ❌ **v7 那一步有真缺陷**（已建 #60） | 我第一版注入是"给 outbox 装 `BEFORE DELETE` 触发器让 v5→v6 炸"，跑出来 **红在我的判据、不是产品**：`outbox.msg_id` 有内联 UNIQUE ⇒ v5→v6 那句"清重复"的 DELETE **一行都删不到** ⇒ 触发器压根不触发 ⇒ `run_migrations` 一路 `Ok` 跑到 v9。⇒ 两条结论：① 那格仍是零判据，且**注入点必须先证明"它真的会写入"**（否则又是个假绿）；② 真正的发现在日志里：**v6→v7 每一步都稳定报 `no such column: id`，而且被设计成吞掉** —— `orphan_group` 谓词写的是 `id NOT IN (SELECT id FROM groups)`，可 `group_reads(group_id, reader_id, last_read_ts)` 与 `pending_group_reads(group_id, peer_id, last_read_ts)` **两张表都没有 `id` 列** ⇒ 那半条"清历史孤儿回执"从上线起就没生效过（注释说的和实际发生的又不一致）。⚠️ 更要紧的待证面：同一个谓词还套在 `group_outbox` / `file_outbox` 上，而这两张表的 `id` 是 **rowid**，拿 rowid 去比 `groups.id`（文本群 id）恒不成立、再拼 `'group:' || rowid` 去比 `conversations.id`（约定是 `'group:' + group_id`）也基本不成立 ⇒ 删除条件可能退化成"**只要 `group_id` 非空就删**"，即在册群的待发文件行也会被清掉。这条在收尾前又做了一次**只读取证，四张表的形状已全部核对**（`awk "/CREATE TABLE IF NOT EXISTS <表> \\(/,/^\\);/" src-tauri/src/db.rs`）：
`groups.id` 是 `TEXT PRIMARY KEY`（群 id 字符串）；`group_outbox.id` 与 `file_outbox.id` 都是
`INTEGER PRIMARY KEY AUTOINCREMENT`（**行号**），且 `group_outbox` 的 `group_id` 是 NOT NULL；
群会话 id 的约定实测为 `format!("group:{group_id}")`（`db/groups.rs` 两处）。
⇒ 拿**行号**去比**文本群 id** 恒不成立、拼 `'group:'||行号` 去比**会话 id** 也基本不成立
⇒ 这两条 DELETE 推定是"**每次有库从 v6 升到 v7，就把 `group_outbox` 全部行 + `file_outbox` 里所有群文件行清空**"，
与那个群还在不在册无关，而且 v7 的注释自称"保守口径"。⚠️ 仍**没跑 RED 落锤**，但形状层面已无悬念 ⇒ **#60 按 P0 处理**，反证它的判据已经写进 #60：给一个在册群 + 一条 `group_id` 非空的 `file_outbox` 行，跑 v6→v7，断言**那行必须还在**。 |
