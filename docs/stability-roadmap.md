...
| **J4** | 群聊：离线成员→上线后补齐 + gossip 收敛 | RC2「永久不同步而单聊正常」的机制最复杂 | 成员重连后收敛到同一集合；撤回按 G-Set 语义不复活；seq 排序权威；不产生重复应用 | ★ **2026-09-26 拆成两半**：**两实例那一半已 AUTOMATED**（`--round=group`：入队时对端进程还没起 ⇒ 只能靠建链后 `flush_group_outbox` 补发，落库/解密/`GroupAck` 清队列/撤回物化/与 1:1 不串味逐格有判据，反向 `-lie` 恰好 7 条红）；**gossip 多跳收敛那一半仍 SIMULATED**（要第三个真实对端，与 §七 最后一格同一套基础设施，改法待拍板 → 12.9）|
...
它证明的是"今天的行为正确"，不是"修了一个缺陷"；§七里真正还剩的空格只有**三实例并发**那一格（⚠️ 这句在 2026-09-26 曾被实测推翻过一次：当时「发送过程中杀进程 × 重启后继续」也没有判据，注入③ 杀的是接收端 —— 补成注入⑩ 之后才重新成立）。L-C 只剩"两个发送者→一个接收者"的真并发与断链才有增量 ⇒ **延后** | 延后（要做就得连三实例一起重开） |
...
**和 §七 最后一格要同一套三实例基础设施** ⇒ 两处欠账是同一块，不是两块。
...
剩余：**三实例并发** ⚠️（§七 唯一未做格，且**修法要用户拍板**，见 12.9。
...
**现在真跑过两族**：文件那一族（八格注入轮）+ **群聊那一族**（2026-09-26 补 `--round=group`，
...
现在每轮两张全屏 PNG（链路建立后 / 两端重启后），判据只钉"落盘 + 不是空图（≥200 KiB，实测 1.77/1.93 MB）"，
...
★ 顺带量到一条**新的运维事实**（#65）：加了截图之后 `test-results/` 一次本地层多写约 44 MB
（12 轮 × 2 张 ≈ 1.8 MB），而**这套产物今天没有任何保留策略** —— 本机实测已累积 **2.6 GB / 113 个 run 目录**。
截图前只有 DB 快照 + 日志 + recv，量级小一个数量级，所以"没人管"这件事是**被 #65 放大出来的**。
处置要用户拍板（删历史报告=破坏性动作，即便那是我自己产的）：
...
证据：正向 25 条全绿；`-lie` 只换"补完该等于哪个摘要"这一份输入 ⇒ **恰好 1/25 红**，
红的就是被换掉语义那一条，其余 24 条（含"两侧 done + 队列清零""无 `.part` 残留"）照常过
...
⚠️ **这一格没有覆盖的一格要说清（别把注释里的动机读成判据）**：注释讲"崩溃不许抹掉已入队"时举的用户现场是
"气泡还在" —— 但本轮**只 seed 了 `file_outbox` 一行**，没有插 `messages` 气泡行，所以"气泡行也不许被抹掉"
**今天是无判据的**。产品的点击路径确实一次写三行（`messages` / `file_transfers` / `file_outbox`，
改小轮⑥ 就是照这个形状复刻的），那是**读码得到的结构性保证**，不是这轮跑出来的证据。
要补的话形状很清楚：入队三行 + 崩溃后断言 `messages` 那一行仍在（同一 transfer 的气泡不许消失）。
