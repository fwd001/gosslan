...
| A-3 文件实验室 | **AUDITED（⚠️ 这行原来写 `DISCOVERED`，是状态机没跟上实测的漂移 —— 2026-09-26 按现状改）**：这张矩阵实际已由 A-1/A-2 交付掉大部分 —— 尺寸阶梯按形状进了本地层（1 KB 单片 / 10 MB 多片带零头，各跑整趟默认轮）、100 MB 由杀进程轮代偿、接收/发送两侧真 `SIGKILL`、冻结/停滞/磁盘/改小/改口/多文件/改名塌在最后各有一轮跨实例判据（逐格看 §12.7，条数由 `check-doc-numbers.mjs` 现算）。**真正还缺的只有一格**：三实例并发（两个 offer 都在任一次 rename 之前到达）⇒ 修法要拍板（A 改名共存 / B 拒第二份，见「待拍板」）。另有一条已登记的边界：A-13（停滞两档在"对端完全冻死"下都够不到、15 s 那一档连日志都不写）| A-1 + A-2 |
...
剩余：**三实例并发** ⚠️（§七 唯一未做格，且**修法已拍板（2026-09-26）＝改名让两份共存**，见 12.9。
