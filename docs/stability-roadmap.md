...
⚠️ 这条观测**不在我手里**：判据与推导值都还在待推提交里（本分支领先 `origin/main` 若干提交，push 由用户执行），
所以"A-6 已收口"只能说到**判据**这一层，"被 Windows 真机观测过"那一层仍未成立。
...
| **A-6** | `#47` Windows 那条腿的清单守卫半失效（"多跑只 WARN"的洞 ⇒ 不在基线里的用例消失了也不红） | **A** | **判据已修**（`209829c` 跨平台完整性判据 + `8c33927` 非空转夹具 + 基线按门控推导）；**残留只有一格**：那份推导名单尚未被一次真实 Windows CI 观测过 | 原判"延后到有 Windows 环境时（技术上只能在 Windows 上 dump）"**已被 `--sync-baselines` 推翻** —— 不需要 Windows 也能把名单推出来并让它可核对。⚠️ 观测那一格不在我手里：判据还在待推提交里，push 由用户执行 |
...
| **A-12** `.part` 字节数冒充「已收完」 | **GUARDS_LOCKED（2026-09-26，`580cff0`，用户拍板候选①）**：按候选①改码 + 单测先红后绿 + 注入⑧两侧一致（`A=failed / B=failed`）+ 交叉自洽断言加回 + 不变量 §17 第三条挂上钩子 + `verify:full` 19 步全绿 | 状态机只走到这里：`DONE` 要等用户 push 后 CI 同一入口再绿一次。边界另记：目录恢复可写之后这一单**不会**自己补完（协议里没有那条码），不算这一格欠账 |
...
| A-6 Windows 那条腿 | **INTEGRATION_VERIFIED（判据层）**：跨平台完整性判据 + 非空转夹具已落地，Windows 名单按门控推导 ⇒ **不再需要"等有 Windows 环境"** | 残留只有一格：**push 之后一次真实 Windows CI 观测**（红了按点名在 Windows 侧 `--update` 转观测值）。⚠️ 这一格不在我手里，push 由用户执行 ⇒ 不许写成 DONE |
