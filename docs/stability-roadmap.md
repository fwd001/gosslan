...
| **A-12** | **「rename 才算完成」被 `AlreadyHave` 短路绕过 ⇒ 发送侧记 done、接收侧记 failed、文件只活在 `.part` 里，而且再也没人补**：接收端续传时按 **`.part` 现有字节数**回答"我已收到 X"；若那次收尾 `rename` 失败（目录不可写/被换走），字节是齐的但**没有 final 文件**。下一次 offer 收到 `received >= size` ⇒ 发送侧走"接收端已完整收下该文件，直接完成" ⇒ 删队列行 + 台账 `done` | **A**（文件"看起来发过去了"实际不存在 + 双侧终态互相矛盾 + 队列行已删=**永久**没有补救机会） | **待用户拍板**（动的是文件收尾判定，属网络核心链；先只登记与钉判据） | 2026-09-26 由注入⑧（`--fault=recv-dir-rotted`：预置**真前缀** `.part` ⇒ offer 不被拒，再把接收目录改只读 ⇒ 只有 rename 塌）实测照出，一轮 13.6 s 内定论：<br>`A 终态 {"status":"gone","aT":"done"} · B=failed · .part=1048576（整份）· final 不存在`。<br>两侧日志互为证据：B 两次报"接收端已有 65536 / 1048576 字节，要求从此续发"，A 第二次收到 1048576 就宣布完成。<br>**与前几格的关系**：⑤（offer 期写不进）只证明"拒绝得干脆"，A-10 是"拒绝时接收方无感"，这一格更糟 —— **两边都给了用户一个结论，而且是两个相反的结论**。<br>判据已落地（注入⑧ 改口轮 4 条断言）：`.part 真被续写 / A 侧终态明确 / attempts 有界 / 接收侧台账不假 done`。**交叉自洽那条今天不成立，所以按 A-11 的先例只用打印钉住分歧，修完之后把这条原文加回断言**：`发送侧宣布完成（队列行被收尾删除或台账 done）⇒ 接收侧必须有整份且 sha256 相等的 final 文件`。修的形状待设计（三个候选，都会动 §三 的受护不变量，故须用户先定方向）：① 接收端在报"已收 N 字节"之前先补做 rename，成功才允许报 `size`；② `received>=size` 之前必须验 sha256；③ 给 offer 答复加一个"完成/仅续传位"字段（属协议变更，最贵）。 |
...
| 注入⑧（改口轮 · 照出 A-12）| `--fault=recv-dir-rotted` = `npm run test:fault-injection:rename`：**先预置真前缀 `.part` 再把接收目录改成只读** ⇒ 往已存在的文件里写照样合法（写权限看 inode），唯一会 EACCES 的是收尾那次 `rename` ⇒ 这正是⑤那条注释点名"要另开一条"的 A-5 形状。**改口轮 20 断言**（默认轮 16 + 本轮 4）+ `-lie` 恰好 1/20 报红。★ 交叉自洽那条今天不成立 ⇒ 按 A-11 的先例**只用打印钉住分歧、不设断言**（这套门禁没有"容忍已知红"这一档，而 `check-doc-numbers` 又要求正向轮次必须接进 local 层），要加的断言原文抄在上一行 A-12 那一格里：<br>`A 终态 {status:gone, aT:done} · B=failed · .part=1048576（整份）· final 不存在` ⇒ **发送侧宣布完成并删了队列行，接收侧只有改不了名的 `.part`**，`AlreadyHave` 按"字节数够了"直接短路，绕过了「rename 才算完成」。<br>⚠️ 三条边界：① 这一轮**没有**接进 `verify:e2e` 本地层（把已知缺陷混进门禁就是制造噪声），只登记为可独立跑的脚本；② 判据刻意**不预设"产品必须失败"**，只判"声称完成 ⇔ 磁盘有整份正确文件"这一条自洽关系；③ 它的 `-lie` 自证明**今天无效**（判据本来就红），要等 A-12 修完再拿它证明判据不空转。 |
