...
| A-2 故障注入 | **E2E_VERIFIED（八格，全部接进 `npm run verify:e2e` 本地层）**：脏 `.part`→错 hash→补齐 / 真前缀必须被续传 / 接收中真 `SIGKILL` 后续完 / 对端失联后解冻自愈 / 接收目录写不进去 / 入队后源文件被改小 / 连续三单其中两单同名 / 字节收完只塌在最后那次改名。每格都有正向绿 + 只翻一个期望值的 `-lie` 反向红（**断言条数不在这里写**：由 `check-doc-numbers.mjs` 现算并与验收矩阵对账）。ADR-0010 已从 Proposed 落到「有八条真跑绿的注入」 | §八 剩余：**DB 锁竞争 / 目标目录变化 / 尺寸阶梯中段 / 三实例并发（两个 offer 都在任一次 rename 之前到达）**。⚠️ 重复帧·乱序帧·旧 attempt 帧·重复 FileDone 这四格**不在"未做"里**：L2 已有具名证据（见 §11 harness 边界行的分层清单），L5 帧级伪造判为**会造出假绿 ⇒ 不可自动**；「单链路断开」同样单机无 root 不可自动（伪造被 route 优先级绕过）|
