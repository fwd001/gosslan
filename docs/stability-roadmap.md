...
处置已拍板（2026-09-26）＝**选 C**：绿轮之后删掉那两份可再生的大文件副本，其余证据（HTML/JSON/日志/DB/截图）全留；
红轮一字不动地全留（删了就等于毁取证现场）。A/B 不采纳。⚠️ **这条今天还只是账，harness 里没实现** ——
实现时的判据形状已在 §十四 定过：删除只在"这轮判定为绿"之后发生，且必须有一条判据钉住
"红轮的副本一定还在"（否则这条清理本身就是个静默吞证据的东西）。
