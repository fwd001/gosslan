...
**现状（这条为什么是硬规则而不是洁癖）**：`Message` 是
`#[serde(tag = "type")]` 且**没有** `#[serde(other)]` 兜底变体；`read_frame` 把反序列化失败
转成 `io::Error(InvalidData)`，reader 循环按连接错误处理 ⇒ 拆链 ⇒ 重连后同一帧再拆，
形成无限循环。也就是说：今天任何"新帧类型"上线，老设备与它**连不上**，而不是"看不懂这一条"。
`ADR-0007` 记录的正是把它从 Proposed 落到实处的决定。
