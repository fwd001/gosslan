...
- Status: **Accepted / 已实现**（Phase 8 已落地：线格式 + 中继接线 + 验收三条的护栏）（2026-09-12 用户裁决：**本版不考虑旧版本兼容**，直接按最优设计做）。
...
### ⚠️ 本节的"不必保证未知 type 不被断链"已被取代（2026-09-20）

当时那条的前提是"没有旧客户端需要兼容"，而 `AI_RULES.md` §12 的这个前提已被仓库自身证伪
（Releases 上 v4.8.2→v4.20.0 是带安装包的正式发布，且本项目无服务器、无强制升级通道）。
"未知 type ⇒ 拆链"的实际后果是**老设备跟新设备连不上**，不是"少收一条消息"。

现行契约是 **INV-P24 + ADR-0007（Accepted）**：帧层必须把未知 `type` 降级成
`Message::Unknown`（忽略 + 节流日志，链路保持），新帧/新语义上线前必须先让"能容忍 Unknown
+ 会报 `protocol_version`"的版本铺开，并按 capability 门控。本节保留的"长度/边界校验、
畸形帧只丢该帧不断链"底线仍然有效。
