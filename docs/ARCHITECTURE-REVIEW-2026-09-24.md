...
### 第 7 步执行进度与两处实测修正（2026-09-25）

| 条目 | 结果 |
|---|---|
| 3「切会话两次 IPC 合成一次」 | ✅ `get_latest_messages`（`f4a4f70`）。等价性由 Rust 侧逐行钉住，顺带修掉一条**已经存在的假绿守卫**（它钉的是字面量 `await api.getMessageCount`，换命令就永远绿灯） |
| 2「三处 O(n) 微改」 | **只有 `peers` 那一处值得做**，另两条按实测判为不做（下面）。✅ `mergePeerList`：内容没变就**一次赋值都不做**，变了也只换真变的那台 + 一条形状守卫钉住三个写入点 |
| 1「`handle_message` 按域分册」 | 未做 —— 纯搬家、短期只降稳定不升体验，排在真正影响用户的那几件之后 |
| 4「`useChatStore` 拆分」 | 按原计划留到最后，且需要先把 1 做完才有清晰边界 |

**实测修正 ①（本条推翻原判断）**：`nicknameOf` 的两次线性扫**不是**性能问题。
量过：一屏 24 行 × 300 台设备（好友+在线）逐帧全查 = **0.066 ms/帧**（16.7ms 预算的 0.4%）。
真正的代价在别处 —— `peers.value = list` 每拍换掉数组**和里面每个对象**，
于是"读过 peers 的渲染"全部失效，与有没有真的变化无关。
所以这一条改成了"引用稳定"而不是"查表加速"：查表是审美问题，重画是用户能看见的。

**实测修正 ②**：`groupReaderIds` 每行返回新数组确实是 prop 身份问题，但它的输入是
`groupReads[groupId]`（群成员数量级、通常 ≤ 几十），单次调用不是热点；
且它只在群聊气泡上被调用。与 ① 同源，先不动，等有一条真能复现的掉帧证据再说。
