...
### 修复（2026-09-26 深夜 · #61 已经跨过坏 v7 的库里那批"活的"幽灵已读回执 · 用户拍板 A）

- #60 修的是"这一步以后别再删错"，但它**救不回已经删错的**：`run_migrations` 只跑 `from >= current`，
  所以任何跨过了 v7 的库永远不会重放那一步 ⇒ 盘上还留着"早已不在册的群"的待发已读回执。
  而 `db/read_receipts.rs::list_pending_group_reads` **只按 `peer_id` 查、不看群还在不在**
  ⇒ 那不是一行死数据，是一只每次对端上线都会被重新捞出来投递的**活幽灵**。
- **修法刻意不给第二份谓词**：把 v7 那 6 条 DELETE 连同它们的锚一起抽成
  `db::sweep_group_orphans()`（返回逐语句错误清单；软失败风格原样保留，不影响启动），
  v7 与新增的 **v9→v10** 共用同一个函数 ⇒ 两处不可能各写各的。
  #60 那次的病灶正是"清理口径只写在一个人看不见的地方"，再抄一份就是把同一个病根留着。
- **判据**（先红后绿）：`v10_sweeps_orphan_pending_reads_in_dbs_that_already_passed_the_broken_v7`。
  种**三种形状**：① 在册群（`groups` 有行 + 会话在）② 群在册但**会话**被删 ③ 两头都不在册的真孤儿；
  版本停在 9 只重放 v9→v10 ⇒ ③ 必须死、①② 必须活、`user_version` 必须落到 `DB_VERSION`。
  ⚠️ ② 才是这格真正的守卫：① 会被"整表不删"混过、③ 会被"整表都删"混过，只有 ② 同时红于两者
  （清理谓词是 `NOT IN groups` **AND** `NOT IN conversations`，只满足一半就必须留下）。
- 🔧 **顺带抓到一条会静默跳过的旧用例**（是它自己变红的，不是我改坏的）：
  `superseded_indexes_are_dropped_and_uniquity_survives` 拿 `user_version = DB_VERSION - 1` 当前置，
  链上一多出 v9→v10，它就退化成"只重放最后一步" ⇒ **删索引那一步根本没跑，用例照绿**。
  现在改钉那一步自己的 `from`，并加一条"这一步必须还在链上"的前置断言；
  这条前置的非空转**实测过**：把步数改成链里不存在的 11 ⇒ 精确红在新加的那句上。
- `DB_VERSION` 9 → **10**。⚠️ 契约图里那句手抄的 `DB_VERSION = 9` 在我 bump 的下一刻就漂了，
  而**全仓没有任何判据发现它漂**（`check-doc-numbers` 不覆盖这个数）⇒ 已换成现算指针
  （`grep -n "pub const DB_VERSION" src-tauri/src/db.rs`），不在第二处抄数字。
- **用户可见面**：升级后首启多一步迁移（v9→v10，正常情况**不打任何日志** —— 那两句
  「孤儿清理跳过一条语句」的告警只在某条语句真报错时才出现，#60 之后按对的锚不该再有）。
  除"已删群的待发已读回执"外不动任何数据，在册群与 1:1 的行一条都不该少（①②两条断言就是钉这个的）。
- **两层现跑（用时打印自门禁自己，不手抄）**：`npm run verify:full` 19 步全绿 / 354.3s；
  `npm run verify:e2e` 14 步全绿 / 493.1s。⚠️ e2e 层**第一次跑在第一步就红**：
  「二进制比源码旧 ⇒ 你正在测旧代码」那道前置守卫拦下了我（改了 `db.rs` 却没重编 release 产物）——
  重编后复跑才是这一层真正的判定。**这条红是守卫在起作用**，不是缺陷。

### 修复（2026-09-26 深夜 · #88 无障碍实测照出的四处"看得见但读不到 / 读得多余"）

- 这一格是 **#78 拍板"走系统无障碍读真窗口"** 的第一批产出：用 `swiftc` 编一个 AX 遍历器对着真实进程读控件树。
  前面那些门禁全绿的时候，这四处一个都不红 —— 它们只有真窗口能照出来。
- **① 会话行的名字不含数量**（`ConversationListItem.vue`）。行是 `div[role=button]` 且带 `aria-label`，
  而 `aria-label` 一旦存在，WebKit 就把后代折叠掉 ⇒ 实测树里那行只有群名，
  **未读数、"有人@我"、蓝色任务数三样全都不读**（屏幕上明明看得见）。
  修法是把这几件事并进**行本身的** `aria-label`（新增 `rowAriaLabel`），复用已有的 `conv.unread` /
  `msg.mentioned` / `todo.openForMe` 三个键，不新增文案。⚠️ 不要指望"读屏自己去读子节点"——实测走不通。
- **② 字母头像被当正文念**（13 个文件 16 处）。`KEEN` / `BRIG` / `HAPP` 这类截断字母作为 `AXStaticText`
  出现在树里，读屏会在人名前念一串无意义字母。同一位置的上传头像早就是 `alt=""`（装饰性），
  字母这一支只是没人管 ⇒ 全部补 `aria-hidden="true"`，并加一条 `designGuards` 判据：
  夹具三例（漏写就报 / 写了就放行 / **多行标签同样判得住**）+ 一条"扫到 0 处就认定扫描本身失效"的防空转。
- **③ 图片的读屏名是 64 位 sha256**（`TodoImageThumb.vue`、`MergeCardModal.vue`）：任务卡与合并转发里的图
  `alt` 直接用了存储文件名（内容哈希），读屏念一长串十六进制 ⇒ 改成 `msg.imageMessage`（"图片消息"），
  文件名仍留在 `title` 给看得见的人。消息流里的图片气泡本来就是 `alt=t('msg.imageMessage')`，
  这两处是漏网的，不是新立的规矩。
- **④ 导航栏自己那行念作"我，我在线（局域网已连接），打开设置"**：`nav.me.openSettings` 模板自带"我，"，
...
### 修复（2026-09-26 · #86 红底白字这一族从未达到本项目的对比度标准）

- 上一轮做无障碍核对时量出来的**既存缺陷**（不是这轮改坏的）：白字压在 `--gosslan-danger` 上
  亮色只有 **3.55**、暗色 **3.16**，而 `--gosslan-danger-hover` 因为混色方向写反（朝白混）更低到 **3.11 / 2.73**。
  ⇒ 站内**每一处**红底白字都不合格：未读徽标的数字、删除/清空按钮、错误 toast、图片预览的删除浮层。
- **为什么以前没人发现**：`tokenContrast` 那张契约表覆盖了"文字 token / 背景 token"和 toast，
  但**从来没有"白字压在一个实底色上"这一形状**；而且检查器遇到 `color-mix` 解析不出就 `continue` 跳过，
  所以 hover 那一行**等于没被读过**（护栏绿、判据空）。
- 改法（用户"下一个按推荐来"）：填充档 `--gosslan-danger` 从系统红压深到 **`#d43d43`**（白字 **4.62**，
  亮暗同值 —— 前景是白字 ⇒ 能压住它的亮度上限由 4.5 决定，与画布无关），
  `--gosslan-danger-hover` 从 `color-mix(… 85%, #ffffff)` 翻成 `color-mix(… 88%, #000000)`（混出 `#bb363b`，白字 **5.67**）。
  ⚠️ 只有"承载白字的填充"压深：`--gosslan-warning` 那类不当白字底的填充**保持系统橙**不变。
  文字档 `-ink` 一律不动（亮 `#cc2418` / 暗 `#ff5548`）。
- **两行 CSS 就能覆盖整族**的前提（实测，不是推断）：`grep -rn "text-\[var(--gosslan-danger)\]" src/` 命中 **0**，
  即这个 token 全仓只当填充用；文字一律走 `-ink`。
- 从此这一族**会红**：契约表新增亮/暗各两条（`#ffffff on --gosslan-danger`、`#ffffff on --gosslan-danger-hover`），
  并且给检查器补上 `color-mix(in srgb, A N%, B)` 的求值能力 + 一条夹具
  （"朝白混 ⇒ 报红；朝暗混 ⇒ 放行"）—— 夹具证明的是**尺子本身**不再静默跳过这类值。
- 红→绿全程实测：加判据后先报 **4 条红**（3.55 / 3.11 / 3.16 / 2.73），改 CSS 后 `npm test` **621 条全绿**；
  另把 `tokenContrast.test.ts` 里那条"填充档保持 Apple 系统色"的**旧结论钉桩**改成新值并写明为什么反转。
- ⚠️ 没做的事：这是**比值级**判据，色觉/对比开关下的观感、以及暗色下这块深红是否"够醒目"仍要真机走查
  （属 §十 Smoke 清单里"运行时观感"那一格，本轮不冒充已通过）。

### 工程（2026-09-26 · A-8/R5：手写建表脚本 `schema.sql` 退役）

- 用户拍板（按推荐 B）：那份"给人看的 DDL"不再保留，表结构只看程序里那一份。
- **删前的最后一次现算**（两条 `grep -oE "CREATE TABLE IF NOT EXISTS ([a-z_]+)"` 取表名再 `comm`）：
  手写那份与 `db.rs` 的 `SCHEMA` **表名集合不等**，手写版缺 `favorites` / `group_files` /
  `group_file_recipients` / `group_recalled_messages`，反方向为空。它自称"与 db.rs 保持一致"是假陈述。
- ★ **这一格的成立方式是"少了一份需要核对的产物"，不是"新增了一条对账门"** —— 写清这一点是为了
  别让下一轮误以为有个 DDL 一致性判据在跑。病灶本身从来不是"少几张表"，而是
  **一份自称事实源、却没有任何一行代码读它**的 DDL（`include_str!` 全仓 0 处命中，实测）。
- 连带改动（一份文件被两个门当名单成员，只删文件会留下悬空引用）：
  `check-change-budget.mjs` 的敏感文件名单去掉它（并在那儿写明**别再加回来**）、`domains.data.mjs` 的域清单去掉它、
  `db.rs` 的注释改口成"本文件即唯一真源"、README / 交接文档 / 架构图 / 领域详解 / PR 模板 / `verify-guards.py` 的
  用例说明一起改。**源码与文档在同一个提交里**：分开提就存在"文档说已退役、文件还在"的中间窗口。
- ⚠️ 边界（没做的事）：`content/store.rs::ensure_schema` 自建的那一张表**仍然**在 `db.rs` 之外，本轮没动
  （动它=动建表路径，不属这一格）；历史 CHANGELOG 里提到 `schema.sql` 的旧条目**不改写**，它们记录的是当时为真。
- 复跑：`grep -rn "schema\.sql" src-tauri/ --include="*.rs"` 只应命中 `db.rs` 那句"曾经有过、为什么退役"的注释
  （本轮实测命中 1 处，就是那句注释）。

### 新功能（2026-09-26 · #84 群任务蓝色徽标）

- 用户要求：把"与我相关、且既没完成也没归档"的群任务数量，以**蓝色数字**同时显示在
  ①会话列表的群条目上、②聊天界面那个任务图标上。
- **判据只有一份**：`utils/todos.ts::openTodosForMe(items, myId)` —— 排除完成与归档
  （归档走既有的 `isEffectivelyArchived`，含"完成满 7 天自动归档"），"与我相关"= 指派含我**或**我是创建人。
  两处 UI 都从 `chat.openTodoByConv` 取数，**不在组件里各折一遍**。
- 取数走一条**新增的只读 IPC** `get_group_todo_messages(conv_ids)`（后端 `db::get_todo_messages_for_conns`
  只把 `kind IN ('todo','todo_update')` 的原始行取回来）。为什么不用内存里的消息算：
  会话消息缓存有上限、冷群根本不在内存里 ⇒ 那样算出来的是"**没缓存的群没有数字**"这种**假状态**。
  为什么不在 SQL 里数：任务真值是多写者 LWW（`(seq, msg_id)` 取最新）+ 两态排除，
  这套折叠今天前端一份、Rust `latest_todo_def` 一份，再来第三份就是本仓反复付学费的漂移形状。
- 配套补了一条索引 `idx_messages_conv_kind(conv_id, kind)`（放在 `db.rs::ensure_post_schema_shape`，
  幂等、每次开库确认一次，不动版本号）。**没有它这条查询会退化成本会话全行扫**，
  而它排在全局那把 `Mutex<Connection>` 后面 ⇒ 大群会冻住整个 UI。
- ★ **这条索引让 `messages` 长出第三个索引，撞上了既存的冻面判据**
  `hot_tables_carry_exactly_the_intended_indexes`（它的问题写在自己注释里："要加第三个索引的人必须先回答
  写路径受不受得起"）。回答方式是实测，A/B **换序各跑一轮**（否则分不清"索引变贵"和"缓存热了"）：
  20 000 行逐条插入 `1043/1032ms → 1264/1224ms` ⇒ **每条消息 +约 10µs**、库文件 `3,682,304 → 4,112,384` 字节
  （+21.5 B/行）；同一个 20 000 行的会话上那条取数 `4.6ms → 1.5ms`（3.1 倍），
  查询计划从 `idx_messages_conv (conv_id=?)`（扫光该会话再回表判 kind）变成命中
  `idx_messages_conv_kind (conv_id=? AND kind=?)`。人手级的消息到达率付得起 10µs ⇒ 判据期望值 2 改 3，
  数字同时记在判据注释里（探针代码**没有留在仓库**：跑完即删，结论只留数字与复跑口径，免得再多一条会飘的计时断言）。
- ⚠️ **过程上的实话**：这条索引落地那轮（#84）只跑了快速层 + 本地 E2E 层就提交了，
  **Rust 层没跑 ⇒ 本文件上面那格判据在 `HEAD` 上红了一个轮次**，是这次 `verify:full` 才抓到的。
  这不是判据太严，是我漏跑了一层 ⇒ 记在这里；今后动 DB 写路径的提交把 `verify:full` 当默认而不是可选。
- 蓝色档加在**全应用唯一的** `UnreadBadge` 上（新增 `tone="info"`），不开第二个徽标组件：
  那枚组件的注释记着"曾经有 5 份手写副本、2 份漏了 `leading-none`"的教训，`designGuards` 也禁手写。
  填充取 `--gosslan-primary-active` 而非 `--gosslan-primary`：白字 11px 数字压主题色只有 3.68:1。
- 无障碍：任务图标的 `aria-label` 在有待办时改成"N 个与我相关的未完成任务"——
  **只靠颜色区别"有活没干完"，对读屏用户等于没有提醒**。
- ⚠️ 顺手量出来一个**既存缺陷并已登记**（见下）：白字压在 `--gosslan-danger` 上只有
  亮 3.55 / 暗 3.16，低于本项目给文字定的 4.5:1 ⇒ 站内每一个未读红徽标上的数字从来就不合格。
  本轮**没有**顺手改这个全站最显眼的颜色，只把实测数字与备选方案写进 `tokenContrast.ts` 的注释，
  并登记成待拍板一格（做成会红的需求会当场钉死所有门禁，故不登记成判据）。

### 新功能（2026-09-26 · #85 自己视角把 @到自己 渲染成「@你」）

- 用户要求：任何 @（群消息正文、任务里关联到我名字的指派/@）在**自己**的界面上显示成「@你」，
  别人看到的仍然是我的名字；并且在自己这一侧要稍微突出。
- 落点在渲染层，**不改发送内容与库**：`utils/linkify` 新增第三个可选参数
  `self?: { name, label }`，命中自己时产出新的段 kind `mention-self`
  （旧段形状一字不变 ⇒ 别人那侧不可能被动到）。标签由调用方传（`i18n` 的 `mention.self`，
  zh `@你` / en `@you`）—— `linkify` 是零依赖纯函数，不该写死中文。
  名字与 `ChatWindow::mentionNames` 里"自己"取**同一处**（本机昵称）；那里原本就有一条
  注释说明为什么不能用 `nicknameOf(自己的 device_id)`（查不到会退化成设备指纹、永远匹配不上）。
- 样式只动**字重与描边粗细**（600→700、1px→2px 内描边），**不动填充色**：
  `style.css` 上方注释写明了加填充会跌破 `chatStyle.mentionHighlightColor` 守的 ≥4.5:1 对比度。
- 判据：`linkify.test.ts` 新增 3 条（自己→换标签且同串里别人不受影响 / 不传 self 时段形状逐字不变 /
  自己是别人名字的前缀时不误判）。先红后绿；红的那一次同时暴露了我自己写错的参数名。
  ⚠️ **视觉层本轮未实机验证**：接线过了 `vue-tsc` 与构建，但没有跑起来截图核对。
- ⚠️ 过程交代：改 `style.css` 时我误删过 `.mention-token` 的 `font-weight: 600`，
  读回原文后已补回 —— 这条记在这里是为了别把它当成"本来就这样"。

### 修复（2026-09-26 · #83 运行日志「合并重复」不再消失）

- 用户反馈"日志合并显示好像没了"。查实是**第三种**情况：逻辑从没被删
  （`git log -S mergeDup` 只有引入那次和一次改配色），但它**几乎不再触发**。
  根因很具体：旧判据要求"连续相邻且逐字相同"，而日志里最啰嗦的那几类行
  每行都带变化量（BLE 退避的 `剩余=…ms`、`分片=n`、`msg_id=`、`seq=`）⇒ 永远不相等；
  再叠加每帧 `[SEND]/[RECV] 已降级为带 trace 才记、logger 调用点变多把连续段打断。
- 合并逻辑从组件搬进 `utils/logFilter.ts`（那个模块存在的理由就是"把用户可见承诺放进可单测的纯函数"）：
  `logMergeKey` 先把 16 位以上十六进制串折成 `<id>`、连续数字折成 `<n>`；
  `mergeLogRows` 允许**非相邻**合并但限制在时间窗内（不限窗会把几小时前那次同类失败并进这一波，
  等于谎报"只发生了一次"）；时间差按**环形**算，跨午夜的两条相邻行不会被算成差 24 小时。
- 取舍写进注释：折叠后显示的是**第一条的原文**加 `×N`（各家日志聚合器同款），
  不是把每次的数字都显示出来。
- 判据：`logFilter.test.ts` 新增 5 条（变化量折叠 / 非相邻折叠 / 超窗必须另起一组 /
  级别不同绝不合并 / 跨午夜合并）。非空转是拿"今天的行为"撞出来的 ——
  把归一化那两行去掉后，恰好描述该缺陷的 3 条报红，用完从备份还原、sha 一致。
...
**门面唯一性这一半其实没有缺口** —— `grep -c "invoke" src/stores/useAppStore.ts` 现算为 **0**（全都走 `api.*`）；
剩下的只是「后发先至」那一半，而那些点多是一次性初始化写。而§十八「不为没有故障证据的热路径动码」⇒ **这轮不改产品码，补一条判据**：给 `useAppStore` 加一条**面积只许变小**的冻面判据（`storeContract` 新增 `useAppStore 的直写面冻在已知四处`：ref 名单 device/shareDir/interfaces + 计数上限）。
`useChatStore` 那条"一条直写都不许"的禁令**没有放宽**，仍只管它自己那一格。

- **冻面而不是清单**：修掉一处仍然绿（那是后续收敛，不该被惩罚），**长出第五处或换成名单外的 ref 才红**。
- **两条断言各管一种 lie，且都实测报红**（不是"看着像会红"）：同一个 ref 再加一处 ⇒ 计数那条红；把一处换成名单外的 ref ⇒ 名单那条红。
  注入用完 `useAppStore.ts` 从备份还原，`git diff` 为零 ⇒ **这一轮生产码一行没动**。
- 顺手把上一条判据的失败文案里"尚未纳入本判据"那句改直（它已被冻面接管），免得留下两处互相矛盾的口径。
- **边界（不许读成"B-5 已收完"）**：那几处直写本身还在，这条只保证**不会变多**；要真去掉 stale 覆盖风险，得逐点走 `utils/staleGuard`，
  而那是一个需要先有故障证据才动的活 —— 已按原状留在 roadmap 的 B-5 行里。
- 门禁：`npm run verify:full` 19 步 exit 0（含「护栏非空转（前端子集）」）。

### Test（2026-09-26 · #79 §十六「本轮每一单都要数得出来」——上一轮留下的那条边界收口）

#70 那一次如实留了一句边界：报告只有一个**标量** `transfer_id`，多文件轮同时那三单里只有一单在报告里有名字。
本轮把它收掉：**判据先写、注册表后写**，产品码一行没动（只动 `scripts/`）。

- 判据（仍只吃**已落盘**的 `summary.json`）：`trace.ids` 必须是数组，且**报告正文里出现的每个本轮形状 id
  都必须已登记**。按形状认、不点变量名 —— 与 #70 同一个理由（手写名单会漏新轮次）。
- 生成侧：所有 trace id 改走一个铸 id 入口，铸出来即登记；反向轮那个"故意不存在"的 decoy 也登记
  （它同样出现在报告里，不登记会让反向轮因契约不合格多红一次，把"红恰好落在断言上"那条证据搅浑）。
- **两面实测**（同一份判据，`--fault=multi-file`；条数只登记在验收矩阵顶部「轮次账」，此处不留数字）：
  正向 `run-2026-09-26T10-34-24-478Z` 功能层全绿 + 契约 ✅ + `exit 0`
  （`trace.ids` 数得出那三张同名不同目录的单）；默认轮 `run-2026-09-26T10-34-12-978Z` 同样全绿。
  反向＝只换判据的输入（id 值一字不变、只是不再登记）⇒ `run-2026-09-26T10-32-38-373Z` 功能层照旧全绿，
  而**报告契约判红、`exit 1`** ⇒ 红来自判据本身而不是应用。
- ★ 这次反向顺带抓到一处**归因错**：保留策略回写后的复判会**复述**首轮已报的缺口，把"首次落盘就缺的那一格"
  打印成"回写弄坏了产物"（同条缺口打两遍，实测 `run-2026-09-26T10-31-25-797Z`）。改成只报**回写新增**的那几条，
  并单独验这一格：把回写改成真的丢字段 ⇒ `run-2026-09-26T10-33-30-518Z` 报"回写弄坏了产物"且**只 1 条**、功能层仍绿。
- 报告契约的自证夹具随之加三格（`npm run test:e2e:report-contract-selfproof` 打印"14 格"）。
- 契约图无需改动：`ARCHITECTURE-MAP.html` 里没有任何关于**报告产物**字段/trace 的断言
  （`grep -o 'summary\.json[^<]*' docs/ARCHITECTURE-MAP.html` 与 `'报告契约'` 均 0 命中），
  图上的 `transfer_id` 全是产品 IPC/事件契约行 —— 本轮没碰那些语义。


### Test（2026-09-26 · #75 §五「群聊 + gossip」最后一格：不在发送者直发名单里的成员，靠中间人收敛）

用户拍板＝**建这一轮，但只挂在发版前**（"日常本地层实测 14 步 / 452.7 s，再挂一个常驻第三实例会拖慢每一次回归"）。
新增显式的一层 `group:"release"` + `npm run verify:release`（`--round=gossip3`），`verify.yml` **不认领**、
`verify` / `verify:full` / `verify:e2e` 三层计数一位没动 ⇒ 落点写进 `docs/acceptance/1.0-release.md` 的发版检查单
（`version:release` 本身不跑门禁，所以只能落在检查单；这一步不做就是"某人记得跑才算跑"）。

**★ 落下来的判据比原计划窄，而且是被实测逼窄的，两条边界都写进了文档与代码注释：**
- 原计划的非空转前置「C 的日志里没有与 A 建链的任何一行」**两次实测都红**（`run-2026-09-26T09-20-42-139Z`、
  `run-2026-09-26T09-24-08-894Z`：C 库里 `lan_enabled='false'`、好友只 B、端点只 B，A 仍打出
  `diag/announce_verified: from=<C>`）⇒ **同机造不出"A-C 无链路"**，拿它当前提这轮会常红。
  正身改成一条不依赖拓扑、且 outbox 任何生命周期都成立的判据：**A 的逐成员直发队列里没有面向 C 的行**。
- 「A 停机后 C 才上线」那个绕法**也不成立**：C 侧 90 s 内 **0 行** ⇒ 中间人只在收到的那一瞬间扇给**当时可达**的邻居，
  **晚到的群成员拿不到补推**。⇒ 这一格判**多跳收敛**、不判"晚到补拉"，后者按实测另记为产品缺口（不替产品许愿）。

**两面（同一份判据）**：正向 `run-2026-09-26T09-34-04-595Z` ⇒ 链式轮全绿（条数只登记在验收矩阵顶部「轮次账」，本步自己 7 条）；
反向 `--round=gossip3-lie`（只翻判据读的那个 `msg_id`）⇒ `run-2026-09-26T09-34-17-793Z` **恰好 4/25 报红**，
而"A 无面向 C 的行"与"B 仍单行"照常绿。

**三实例的地基顺带补齐两处真缺陷**：
① 备份/还原、`after-*.db`、日志关联此前**点名 A/B** ⇒ 用户本机原有的 `gosslan-3.db` 一旦存在就根本不在保护范围内；
现在一律按"本轮可能被启动过的实例清单"走（跑完打印「已还原用户原有实例库 3 个」）。
② §十六 报告契约：`check()` 的 `expect`/`actual` 为 `undefined` 时被 `JSON.stringify` **整个键丢掉**
⇒ "预期/实际"两栏凭空消失（正是那个反向 run 里三条的样子）。已改成显式渲染 `<无此行/undefined>`。

★ 契约图 `ARCHITECTURE-MAP.html` 那张 E2E 卡原文写"正向轮**全部**接进 `verify` 的 local 组"——
新开 release 层之后这句就是假的（§十三意义上的漂移，本轮一起改口，并把"要第三个真实例那一轮挂在发版前层"写进卡片）。
改完 `node --check` 内嵌 script + `check-doc-numbers` 复跑均 ✓。

零生产码改动（只动 `scripts/`）。

### Fix（2026-09-26 · #54 两份同名文件同时在途，后一份会**覆盖**前一份 —— 两行台账都是 done，而其中一份字节已经不在了）

§七 那张矩阵里唯一没做的"两个 offer 都在任一次 rename 之前到达"（用户拍板＝改名让两份共存）。
**RCA 推翻了"要三实例才能验"这个前提**：根因是**单聊收尾沿用了 `begin` 时定的名字** ——
`make_receiver` 里 `final_path = unique_path(...)` 在磁盘上还没有同名文件的那一刻就定死了，
于是同名的两单拿到**同一个** `final_path`；收尾那次 `rename` 在 POSIX 上直接覆盖先落地的那一份
⇒ 两侧各留一行 `done`、两个气泡都在，其中**一个指向已经不存在的字节**（最难发现的一类丢法）。
对照事实：**群聊收尾早就有这半格**（`transport.rs` 落盘前"名字被占走就换名"），
而它那条注释写着"单聊本来就是在写盘时才定名 ⇒ 没有这个竞态"——**这句与代码不符**，本轮一并纠正。

**修法（一处，最小）**：`finish_receiver_into` 在真正 `rename` 之前再确认一次 ——
`final_path` 已被占走就按同一目录重新取唯一名，台账与返回值都写**实际落地那个路径**；
不拒收、不改协议、不动 begin 的定名时机。
**判据先行**：`two_same_named_transfers_do_not_overwrite_each_other` 直接驱动生产收尾函数
（不是影子副本），夹具只复刻"两次定名给出同一个名字"这一件事。
先跑**红**：`assert_ne` 失败、两行报同一个路径；修后**绿**，同模块 **66 条 0 失败**（无回归）。

**★ 同一句"剩余：三实例并发"抄在 4 处，我只改了 1 处**（§十三 意义上的漂移，本轮按处数改全）：
`A-2 行`、`注入⑦ 边界行`、`"覆盖的边界"行`、`§收尾四件事清单` —— 全仓 `grep 三实例` 逐处回锚点，
并把"两处欠账共用一套三实例地基"这条**已被这次收口推翻的推理**一并改掉（现在只剩 gossip 多跳一处要用它）。
契约图 `ARCHITECTURE-MAP.html` 的「磁盘目录」节点原本只写 `unique_path 防同名互相打乱` ——
**这句是同一个 bug 的文档形态**：它没说出保证在**哪两处**执行，只读图的人会把 begin 那一次当成全部。
改成「**两处**：begin 定名 + 收尾 rename 前再确认（只在 begin 定名挡不住两份同时在途）」。
★ 两条自证：`node --check`（内嵌 script 语法）+ `node scripts/check-doc-numbers.mjs` → ✓ 6 份活文档对账通过。

**过程中我自己写坏过文档一次**（如实记账）：用一个按锚点切片的 node 脚本做批量替换时 `keepB` 语义写反，
两处变成**新旧句子并存**、一处把下一条目的 `- **M-1′…` 行首整段吃掉 ⇒ 半截项目符号；
脚本六处全打印 OK，**三处损坏都是跑完自己 grep 才发现的**。补救后再跑一次"落盘前恒等判据"，
它判红拒写（exit 3）—— 那次是**假阳性**（新句里正当引用了被禁的旧措辞）⇒ 已把该判据换成结构锚（句尾/行首）。
教训：脚本化改文本的"成功"判据必须是**旧串不再存在**，不是"新串已插入"。

**门禁（`b04287b` + 本轮改动，现跑现读）**：`cargo build --release` 1m43s（第一次跑本地层被前置闸拦下 ——
`✗ 二进制比源码旧 ⇒ 你正在测旧代码`，那道闸是故意的，不是回归）⇒ 重编后
`npm run verify:e2e` = **14 步 / 452.7 s / ✅ 那一组通过**，其中注入⑦ 那条
「同名不许互相覆盖：接收目录里这张名字必须出现两次」✅；`check-doc-numbers` ✓。

### Test（2026-09-26 · #71 §十六 产物保留策略选 C 落地：只有判绿的轮才删那两份可再生的大文件副本）

拍板选 **C**（用户 2026-09-26）后落进 harness：每轮 `finally` 里，**只有这轮真判成绿**才删
`RUN_DIR/src`（发送用的原件拷贝）与 `RUN_DIR/recv`（收到的文件）—— 这两份的字节数与哈希已写进 `summary.json`，
随时能重跑出来；日志 / DB 快照 / 截图 / summary 一律留。实测此前 8.9 GB / 216 个目录里约 98% 就是这两份。

**判据是纯函数 + 四格自证**（绿⇒删这两样 / 红⇒不删 / 反向模式⇒不删 / 报告自己不合格⇒不删），
自证任一格不成立就**一律不删并把这轮判红** ⇒ 这条清理坏的方向只会是"多留"，不会是"删掉取证现场"。
...
接着 #69 那条契约判据往下查，抓到更隐蔽的一格：报告里挂"日志关联"用的过滤键是**手写四个变量名**
（`msgId` / `xferId` / `xferId2` / `xferId3`），而 6 个注入轮各自在函数里另造 id
（`xferId4`/`xferId5`/`xferId6`/`xferId7`/`xferId8`、群聊轮的 `gTextId`/`gRecallId`/`gText2Id`）
⇒ **注入那一单在自己的日志里有 132 行，报告里却是 0 行**，而同一份报告的标量 `transfer_id` 又指向本轮主单
—— 沿链追踪在最需要它的失败轮上指到了**错的那一单**。
实测锚点：`run-2026-09-26T01-42-36-986Z` 的 `instance-A.app.log` 里被注入的
`e2e-i-2026-09-26T01-42-36-986Z-dqy5zx` 计 132 行，报告侧 0 行。

**修法（一处，仍不动产品码）**：过滤键从"点名列举"改成**按形状匹配** ——
`e2e-\S*<本轮 ISO>`（ISO 里的正则元字符先转义）。这一轮所有 id 都带同一个 run 时间戳，
所以按形状匹配天然覆盖"未来新造的第 N 个 id"，而手写名单下一次漏掉只是时间问题。
★ 这正是护栏记忆里那条通用教训的具体一次：**点名式/存在性断言只算半个守卫**，
凡"这一族的成员还会增加"的地方，判据要锚在形状上而不是成员名上。

**实测（2026-09-26）**：改后重跑 `--fault=recv-dir-rotted-lie` ⇒ 恰好 **1/24 条断言报红**（反证照旧成立），
报红那步的日志关联 **A 侧 8 行 / 含 `e2e-i` = 8**、**B 侧 6 行 / 含 `e2e-i` = 1** ⇒ 过滤键真读到了被注入那一单；
契约判据 `报告契约不合格` 计数 **0**；快速层门禁 **13 步 / 8.8 s / exit 0**。
⚠️ 如实留下的边界：报告里那个**标量** `trace.transfer_id` 仍指本轮主单，
"每一单都有自己那条 trace"这句还没做到（§十六 的 Trace 贯穿是待办，本轮只修"日志挂错对象"这一格）。

### Test（2026-09-26 · #69 §十六 报告契约改成机器判据，顺手抓到"没跑的步骤被渲染成 ✅"）

上一轮我核对 §十六 产物字段时先误报了一格（"缺日志关联" —— 那格只在有失败步骤时才挂上去，
`--negative` 那轮 0 命中行本身就是结论）。核对过程中真查出来的是另一格：
`summary.html` 步骤徽章是一个三元式，`verdict` 缺失（fail-fast 跳过的步骤）时落到 else ⇒ **显示成 ✅**。
报告把"根本没跑过的步骤"标成通过，正是 §十 禁止的"把没有测试伪装成 PASS"，只不过这次被伪装的是报告自己。
实测锚点 `run-2026-09-26T06-53-47-012Z/summary.json` 第 5、6 步只有 `{name}` 一个键。

改的两格都在 harness 里，**产品码一行没动**：① 落盘步骤强制有终态（没跑到 = `NOT-RUN`），
徽章加一档 `⛔ 未跑`，页脚写明未跑不许算进通过格；② §十六 那串字段名从"人肉核对过一次"变成
对**产物**的判据（`verdict`/`trace.msg_id`+`transfer_id`/`duration_s`/步骤终态/断言的预期·实际/
"报 FAIL 且有失败步骤就必须带日志关联"），每轮 `writeReport` 后把刚写那份读回来判，
不合格就在清理之后盖过退出码改判 1 —— 包括反向模式那条"按设计退 0"。
判据自证 11 格 `npm run test:e2e:report-contract-selfproof`；`--report-contract=<run 目录>` 可用同一份判据读任意报告。
★ 反向自证照旧"只换判据的输入"：注释掉生成器里 `trace` 那一行 ⇒ 断言层仍打印"反向自证通过"，
而报告契约判红、退出码从实测 `0` 变实测 `1`；恢复后同一轮实测契约 ✅ + 退出 `0` + HTML 出现 3 处"未跑"。
⚠️ 判据只在每轮生成时生效，不追溯历史 60+ 目录（旧代码写的报告本来就没这些字段）。

### Test（2026-09-26 · #68 注入③ 的期望值改取"确认已死后冻结"的盘上真值 —— 红的是判据自己的读数窗口）

本地层跑红一次：注入③「重启后必须从**盘上真实字节数**续发」**预期 4,194,304 / 实际 4,456,448**，差恰好一个 256 KiB 片。
取证到run 产物级别：重启后的 B 自己打的是「接收端已有 **4,456,448** 字节，要求发送端从此续发」，A 照做
⇒ **产品服从的是盘上真值**，红的原因是这条判据把期望值取在**按下 `SIGKILL` 之前**那一次 `stat` ——
从快照到进程真死之间 B 还在收片（且写入也要一会儿才在 `stat` 上显现），两者之间本来就有一扇窗。
这与 #66 收口的「拿对端产物当本端同步点」同族，只是这一格在**注入前后的基准读取时机**上。

**修法（判据层，不动生产码、不改断言条数）**：期望值改取**确认已死之后**再读的那一次 ——
`.part` 只有 B 会写，它一死这个数就永久定格，等式两边才真的可比。两个读数都打印出来当探针
（`按下 SIGKILL 前读到 X，确认已死后冻结在 Y（差 Y−X）`），下次窗口再张开就是可见的实测而不是猜测。
⚠️ 「窗口必须真打中」那条**仍按决策时刻的快照判** —— 它问的是"我下手时是不是真的在半路"，
与"产品有没有服从盘上真值"是两件事，不能共用一个数。
顺带扫同族形状：**注入⑨ 本来就取在冻结之后**（这就是它从没红过的原因），只补了一句 settle ——
`kill("SIGSTOP")` 是异步生效的，紧贴调用去读仍可能读到"最后一片还没落地"的那一刻。

**实测（2026-09-26，同一份判据连跑三次）**：正向两次 **25 条全绿 / exit 0**，三次打印的差值都是 **0**
⇒ 今天这三下没张开窗口（那扇窗的存在由上面那次红证实，不由这三次绿否证）；
`-lie` **恰好 2/25 红**且红的就是被换掉语义的两条（续发点 7,340,033 vs 实测 7,340,032、摘要对不上）
⇒ 换基准之后这条判据**照样读真数据**，没有变成同义反复。
证据：`test-results/run-2026-09-26T06-28-35-916Z`（红的那一次的 A/B 双侧日志）。

### Test（2026-09-26 · #67 注入⑩「发送端被杀轮」：§七「发送过程中杀进程 × 重启后继续」第一次有判据）

先记一条**我自己上一轮写错的账**：CHANGELOG/roadmap 里那句「§七 只剩三实例并发」是错的。
注入③ 杀的是**接收端**，全仓没有任何一轮从"**发送端自己崩了**"这一侧看过 ⇒
§七「发送过程中杀进程」+ §八「本端重启」+ §九「数据生命周期：运行→退出→重新启动」那一格当时**也是空的**。
新增 `--fault=sender-kill-mid`（+ `-lie` 反证、`test:fault-injection:sendside[-selfproof]`、门禁 local 层点名）。

**这条判据钉的核心不是"能不能续传"**（②/③ 已证），而是**一次崩溃不许把「已入队」这个事实抹掉**：
`file_outbox` 那行是「先入队再投递」这条不变量的载体 —— 它若因发送端崩溃变成 `done` 或直接消失，
这一单就**永久没人再发**，而用户界面上的气泡还在，是最难发现的一种丢法。
时序：判据自己入队 → 50 ms 自旋抓到 B 的 `.part` 真的在长 → `SIGKILL` **A** → 死透后再等 20 s
（这 20 s 是"发送端根本不存在"的时间）→ 重启 A → 等它自己补完。

**实测（2026-09-26）**：在飞 `.part=5,242,880 B` 时杀 A，那 20 s 里 B 侧无终名文件、B 台账 `failed`、
A 台账 `active`、**A 队列行仍是 1**（崩溃没抹掉它）；A 重启后 **12.6 s** 从盘上那点字节续到整份
104,857,600 B，内容摘要等于源、两侧 `done`、队列清零、不留 `.part`。
证据：正向 **26 条全绿**；`-lie` 只换"补完该等于哪个摘要"这一份输入 ⇒ **恰好 1/26 红**，
红的那条正是被换掉语义的那一条，其余 25 条（含"两侧 done + 队列清零""无 `.part` 残留""崩溃后气泡还在"）照常过
⇒ 这条红不是顺带塌的，是精确落在判据上的。零生产码改动。

★ **入队要写三行，不是只写队列那一行**（提交之后**同一轮内**补的加强，性质是"证明今天的行为正确"，不是修缺陷）：
上面那句动机讲的是用户现场 ——「队列没了」和「气泡没了」在 UI 上是**两个不同结局**（前者=永久没人再发、
界面上还看得见这一单；后者=用户连"曾经发过这一单"都看不见）。只 seed 队列那一行就只能证明前一半，
所以照⑥ 已有的形状把 `messages` 气泡行与 `file_transfers` 台账行一起补进来，崩溃窗口各钉一条。
它的非空转证据用同一种便宜证法：**只把种下去的那个气泡 `msg_id` 换成 decoy**（判据读的输入变了、判据没动）
⇒ 恰好 1/26 红、红的就是"崩溃后气泡必须还在"那一条；跑完把文件按字节还原（`diff -q` 通过）。

**顺带被守卫自己抓到一处我写错的登记**：我把反证轮名 `--fault=…-lie` 写进了 `verify.mjs` 的 local 层步骤说明里，
判据 D 当场判红（"local 层点名了反向模式 —— 它们预期红，进门禁就是把整层静音掉"）。
守卫是对的，改的是我：反向模式只登记在 `package.json` 与验收矩阵，**不进门禁那一层**；
已把这条约束本身写进那步的说明里，免得下一个人再犯。

### Test（2026-09-26 · #66 注入⑨「停滞轮」：字节在飞时冻住对端 + §19 Relay 那一格改判）

§七「发送方提前放弃 / 接收方仍在线」× §19「文件·断线」这一交叉此前**零跨实例判据**。
新增 `--fault=stall-mid`（+ `-lie` 反证、`test:fault-injection:stall[-selfproof]`、门禁 local 层点名）：
判据自己入队 → 自旋抓到 `.part` 真的在长（实测 7,077,888 B）→ `SIGSTOP` 接收端 → 冻 70 s → `SIGCONT`。
**三条实测**：① ★ 不变量「接收端真实进度」第一次有跨实例机器证明 —— 冻结期间盘上字节**一字不涨**、
终名文件不出现、两侧台账都不是 `done`；② ★ 我原本要钉的 `[STALL]`（60s 停滞放弃）**一行都没出现**，
先兑现的是 chunk loop 的 `[FILE] COMPLETED … ok=false`（队列退回 `pending`、`attempts=1`）
⇒ 那一格改成只打印不判据（钉死任一条未兑现的出口就是假红）；③ 解冻后确实补到源内容、两侧 `done`、
队列清零、半截 `.part` 不残留，但**解冻→落地 51.6 s**（远慢于冻结轮④ 的 0.5 s：这一单要先退避再重拨）。
证据：正向 25 条全绿；`-lie` 只换"补完该等于哪个摘要"这一份输入 ⇒ 恰好 1/25 红。零生产码改动。
⚠️ 两个**只打印、不设判据**的量随环境变：抓到在飞字节实测过 1.8 MB / 7.1 MB（看自旋撞上 SIGSTOP 的时机），
解冻→落地实测过 5.5 s / 28.6 s / 51.6 s（看这一单是先退避再重拨、还是原链路直接续）
⇒ 把这种数写成判据就是把机器负载当产品行为。

**就地纠错**（全是我自己上一轮写下的）：
① 冻结轮④ 注释里那句"在飞窗口等不到冻结生效"只对"手动掐时机"成立，自旋抓法已被 ③/⑨ 两次证明做得到
⇒ 注释里标清两轮的分工，不许互相冒充结论；
② #65 那条截图判据按体积定阈值（200 KiB）被实测推翻：桌面接近空白时同机只截出 ~104 KB
⇒ 每轮 E2E 在起跑前被"先修判据再跑轮"拦停，红的是判据不是产品。改为**结构判据**
（PNG 签名 + IHDR 宽高 > 0），体积只留挡桩文件的下限，并给自证补第 4 格（宽高为 0 的桩 PNG 必须判假）。
③ ★ **停滞轮自己那条"终局"判据被本地层跑红，红的是同步点选错了**：我拿"B 的终名文件出现"当 A 该终态的时刻，
可 A 的 `done` + 删队列行是**收到 `FileCompleteAck` 之后**才写的，而那条 ack 在 rename **之后**才发
⇒ 天然顺序就是"先见 B 落地、后见 A 终态"。本地层第 12 步实测：解冻→落地 5.5 s 的那一刻读 A
⇒ `{A:active, outbox:1}` 判红，而同一步里 B 已 done、字节与 sha 全对、无 `.part` 残留
⇒ **不是产品分叉，是判据读早了**（单跑一次绿纯属运气：`-lie` 轮打印出 **B 落地 → A 终态 14.6 s**，
而改完之后同一格又打印出 **0.0 s** ⇒ 这段滞后**不是常数**，"这次赶上了"完全不构成可靠）。
修法：新增 `waitSendTerminal()` —— 有界等发送侧自己走到终态（60 s = 心跳 5 s + 队列退避 5 s 留足余量，
到不了才判红），并把等待耗时打进报告。**同一形状顺着这个 helper 把 harness 扫尽**：无条件断"两侧 done"的默认轮 J2、杀进程轮⑥、冻结轮④、
停滞轮⑨（⑥ 原本用 `sleep(15_000)`、②续传轮用 `sleep(12_000)` 把这类竞态**藏住**而不是解决它），
再加脏前缀轮①（它那两条是蕴含式后件 ⇒ **只在补齐分支才等**，不补齐那一支今天仍允许停在重试中）
⇒ 全部改走同一个 helper，
`check(` 调用点一条没增减（断言数由判据 C 现算复核仍对得上）。
⇒ 通用一条：**跨实例判"终态"之前先问"这一侧的终态是被谁点亮的"**——对端产物不是本端的同步点。

★ 顺带改判 §19「网络·Relay」：这一格缺的**不是**中继服务器（就在本机 `gosslan-relay-server`），
而是"一条被封的直连"—— `relay.rs:104` 把已有**任何**链路（不分通道）的对端整个跳过，
同机两实例靠 LAN 一定连得上，且仓库里没有只关 LAN 发现的开关（`discovery_mode` 不存在）
⇒ 真跑需要 root 防火墙或第二台真机 ⇒ 记 **Smoke-9**（含可观测判据），不升 AUTOMATED。
⚠️ 同一轮把 §七「发送方提前放弃」与「用户点取消」拆开：后者要 IPC 调 `cancel_file_transfer`
（内存信号 + `AppHandle` emit），零改产码的 harness 够不着 ⇒ 那一格仍待驱动层或 Smoke，**不许用前者冒充**。

### Test（2026-09-26 · #65 §十六 报告里的 `screenshots/` 这一格第一次真的有产物）

- 之前的事实：`test-results/run-<ts>/` 每次跑都生成 summary.json/html、两端 app.log、
  `after-*.db`、`recv/`、`backup-*/`，**只有 `screenshots/` 一直是空的**（总指令§十六点名列了它）。
- 现在：每轮在两个时刻各抓一张全屏 —— ①「起 A/B 并等链路真的建立」之后（两端窗口都在桌面上、链路刚成立），
  ②「两端重启」之后（§九「数据生命周期 → 重新启动」那一格的界面证据）。产物落
  `test-results/run-<ts>/screenshots/`，`summary.json.shots` 写相对路径，`summary.html` 内嵌图集。
- ★ **判据只管它管得起的那件事**：截图**真的落盘且不是空图**（文件存在 + 体积 ≥ 200 KiB；本机实测 1.77 / 1.93 MB）。
  "界面对不对"**不是机器判据**（没有像素级断言），那一半仍按 §12.6 记结构级/MANUAL ——
  把"抓到图"写成"界面被验证"就是§十禁的假 PASS。
- 判据自己的账（照 #63/#64 同一形状）：`scripts/e2e-shot.mjs` 自带三格自证，**跑轮之前先跑**，
  另有独立入口 `npm run test:e2e:shot-selfproof`（不需要 release 产物）。
  ① 16 字节的假 PNG 必须判假；② 不存在的文件必须判假；③ 真截一张必须判真。
- ★ 反证：`E2E_NO_CAPTURE=1`（把采集器关掉 = 判据读的那个输入变了，判据本身没动）⇒
  默认轮 **恰好 1/18 报红**，红的就是这条，而同一步里原有两条判据照常绿。
- 顺带修掉我自己刚写出的一个假证据：关掉采集器时自证打印"能判假 + 能判真"，
  而"能判真"那一格当时**根本没跑** ⇒ 改成两种文案分开报（跳过就说跳过）。
- ⚠️ 边界：采集器**只实现了 macOS**。Windows 腿接上之后这一格会是**红**（不是静默跳过）——
  那是明着的欠账，要在 Windows 上另做采集器（PowerShell CopyFromScreen 之类）才算补上。
### Test（2026-09-26 · #64 §19「网络」Routed 这一格第一次有跨实例判据 —— 顺带把散在 11 处的断言条数收成一处）

- **缺的不是判据强度，是"这件事有没有人看着"**：双实例 harness 每一轮都在两端预置
  `routed_endpoints`，那条手动配置的端点拨号实际上是**每轮第一条链路的来源**（实测 11/11 轮里
  至少一侧打出 `建链 … path=routed`），而默认轮的"等链路建立"步骤只等
  `peer=<对端 id>` 出现在日志里 —— **LAN 广播也能让它出现**。⇒ 把 Routed 拨号循环整个改坏，
  12 轮本地层照样全绿。
- **加的判据**：这一对实例之间**至少一侧**打出过 `建链 peer=<对端> path=routed`（两侧合计 ≥1）。
  刻意不按侧断言：谁先拨到谁记 routed、另一侧只看到入站连接，11 轮实测里 A=routed/B=lan 与
  A=lan/B=routed 两种都出现过 ⇒ 按侧写就是下一次漂移。
- **反证是"换判据读的那个输入"，不是换判据**：`E2E_NO_ROUTED=1` 只把两端预置的
  `routed_endpoints` 清成 `[]`（生产码没动过）。跑出来新那条超时红，
  **而同一步里旧的那两条 `peer=<id>` 判据照常过** —— 这句就是"以前看不见"的实测证据。
- ⚠️ **这一格没有变绿**：出口仍是回环、LAN 同时在跑，走的大概率还是 LAN。它证明的是
  "以手动配置端点驱动的建链真的成立"，不是"非 LAN 出口真跑过一趟"。§12.3 本格态保持 ⚠️，
  欠的那句话原文照抄没改。
- **顺带修的结构性漂移**：一条断言 +1 就让 `check-doc-numbers` 判据 C 报出 12 处手写数字漂移
  （16→17、20→21、22→23…）。按判据 C 自己给的办法**把数字从叙述里删掉**，收成验收矩阵顶部
  一处「轮次账」；同时删掉「lie 恰好 1/N 报红」这类分母 —— 红几条取决于当时翻掉哪个期望值，
  写成常数下次必漂。以后加一条判据只改那 10 行。
- 零生产码改动（`scripts/` + `docs/`），零用户可见行为变化。

### Fixed（2026-09-26 · #63 双实例 E2E 的"等就绪"判据会被日志轮转吃掉 —— 红的是判据，不是产品）

- **用户可见后果**：**没有直接的**。产品行为一行未改；塌的是**证明它的那层** —— harness 拿日志里的一行当
  "这个实例真的起来了 / 这一单真的被投过"的证据，而那一行会因为应用自己的日志轮转而**整行消失**。
  表现为本地 E2E 某一轮在「等 boot 完成行」上超时 20s 报红，fail-fast 之后**连带跳过后面 9 轮**
  （含当天新加的群聊轮），看起来像"产品坏了"，实际是判据看不见证据。
- **根因（逐行读 `logging.rs:296-324` + 事后核对档）**：日志是**跨启动追加**写的，每次 append 前量一次
  当前文件大小，一旦超过 `MAX_LOG_FILE_BYTES` = 512*1024 = **524288 B** 就 `remove(.old.log)` +
  `rename(.log → .old.log)` —— 旧档被**覆盖**、当前档从零写。判据读的是"当前档最后 60 行"，
  于是 boot 行刚写进去就随轮转被挪走。实测账目：红那一轮 `gosslan-2.log` 共 21 行、boot 行 **0** 条，
  该行的三条历史都在 `gosslan-2.old.log` 的第 4255/4260/4277 行。
- **第一次修法仍然红（这条最值钱）**：只把读取改成"跨 `.log` + `.old.log` 数次数、和启动前的次数比"不够 ——
  把当前档垫到 524250 B（距上限 38 B）再跑同一轮，boot 行落进 `.old.log` 的同时那份 `.old.log`
  **覆盖了上一份** ⇒ 跨档计数从 6 **掉到** 4 ⇒ "比基线大"永远不成立 ⇒ 同样 20s 超时、EXIT=1（实测）。
- **真修法**：判据读的那份日志必须**从本次启动起归 harness 所有**。每次 launch 前 `stashLogs` 把该实例的
  `.log` / `.old.log` **整份移动**进 run 目录（是移动不是删除，事后照样翻证据），本轮写的行只能在当前档里；
  再配跨档读取兜住"同一次启动内又轮转"，`bootReady` 仍按"比启动前基线多"判以挡住残留旧行造成的假绿。
- **判据层自己也终于有了回归**：新增 `scripts/e2e-logtail.mjs`（5 格夹具 + 自证），**每轮 E2E 启动前先跑**，
  红了直接以「判据坏了」退出、不再留一个 20s 超时让人猜；独立入口 `npm run test:e2e:logtail-selfproof`。
  夹具里两格是**钉住今天这两条红的反面**的：只看当前档必须看不见（否则夹具压根没在复现 bug）、
  不 stash 只比基线必须仍判不出就绪。非空转由三次变异证明：读法改回今天线上那套 ⇒ 红 2 格；
  去掉基线比较 ⇒ 红 1 格；把 `stashLogs` 改成空操作 ⇒ 红 3 格。全部是断言失败，不是编译错。
- 顺带抓到并修掉实现里一个真实缺陷：两份档相加时没补换行，会把旧档最后一行和当前档第一行
  **黏成一行**（夹具⑤第一次跑就靠这条抓到它：两行 boot 只数出 1 行）。
- 文档同步：roadmap §11.4 加了第 13 个陷阱、§11.5 生命周期补了清档这一步、§12.7 补"判据层自己的账"。
  **零生产码改动**（`src-tauri/` 一行未动）。

### Test（2026-09-26 · #62 群聊这一族第一次跨实例真跑）

- **补的是哪一格**：§九「群聊：创建 / 同步 / 发送 / 成员离线 / 重新上线 / 撤回」在跨实例层面此前是
  **零判据** —— 双实例 harness 从没建过群（`grep -c group` 只命中 `file_outbox` 的列名）。
  新增 `--round=group`：两端预置同一份群记录与群密钥，A 排三条群消息（正文 / 撤回 / 第二条正文），
  **入队时接收端进程还没起** ⇒ 只能靠建链后的 `flush_group_outbox` 补发，于是顺带覆盖了
  §五 点名的「群聊 + 离线成员重新上线」与「聊天 + 群聊 + 文件 共用同一对实例」两格组合。
- **群聊轮 11 条判据**（正向全绿）：三条各只落一行、落的是群会话（`conv_id` + `conversations.kind` 双查）、
  未被撤回那条必须是**解密后的明文**、`sender_id`/`seq`/`status=delivered` 照信封、
  撤回把目标物化成 `kind=recalled` + 空正文**但不删行**（G-Set）、撤回事件自身也留一行、
  三条群消息一条都不许出现在 1:1 会话、A 侧 `group_outbox` 必须被 `GroupAck` 清成 0、
  A 侧气泡不许被误判 `failed`。反向 `--round=group-lie` 只换判据读的那个 id ⇒
  **恰好 7 条红、4 条与 id 无关的保持绿**，证明这 7 条读的是真落库行。
- **两条如实记下的边界**：① 群 payload 不像 1:1 会被 re-seal（`flush_group_outbox` 原样发），
  所以信封由 harness 自制 ⇒ 这一轮证明的是**接收侧能解能落能收敛**，不证明发送内核自己怎么组信封；
  ② harness 写的是"入队形状"、没执行产品的撤回命令 ⇒ **发送侧本地物化不设判据**。
- 第一次跑（27 条里 1 条红）红在**我自己的判据**：「必须是明文」与「撤回后必须为空」读的是同一行同一列，
  互斥。按实测契约拆成两条独立判据（多一条不被撤回的正文）后才全绿 —— 红的是判据，不是产品。
- 零生产码改动。

### Fixed（2026-09-26 · #60 P0：v6→v7 迁移会把**在册群**的数据当孤儿清掉）

- **用户可见后果**（RED 已实测，不是推定）：任何一个还停在 v6 的库升到 v7 时，
  `group_outbox` 的全部行 + `file_outbox` 里所有群文件行会被清空 —— 与那个群还在不在册无关。
  表现为升级后"给离线群成员发的消息 / 群文件"静静地不再补发，而且没有任何提示。
- **根因一句话**：那条清理的判据拿**行号**去比**文本群 id**（`id NOT IN (SELECT id FROM groups)`），
  两个域永不相交 ⇒ 条件退化。另两句套在 `group_reads` / `pending_group_reads` 上更直接：
  这两张表**没有 `id` 列** ⇒ 稳定报错，而该步是软失败风格（只 `eprintln` 不上抛）
  ⇒ **从上线起一次都没生效过**，没人看得见。
- **修法一处**：判据改锚 `group_id`（四张表都有这一列）。谓词形状、软失败风格、事务边界、
  `DB_VERSION` 全部未动 —— §十八：不借修 bug 改迁移框架。
- **判据**：`migration_tests::v7_orphan_cleanup_keeps_live_group_rows_and_drops_true_orphans`
  （11 条断言：在册四张表的行必须活 + 孤儿行必须死 + 1:1 待发行不许被波及）。
  ⚠️ 两面都钉才不空转：只断"活下来"会被"整表不删"混过去；而那两张没有 `id` 列的表
  恰好**只有孤儿侧断言**能证明语句真跑通了。先跑红（`m-live` 计数 0 / 期望 1）再改码，
  逐条损伤也是实测出来的（同一份同形状临时库上原样跑那 6 条 DELETE 打印行数）。
- 🔶 **留下的账 = 新格 #61（当时刻意不扩）**：已经跨过 v7 的库不会重放那一步，
  所以它们盘上仍留着"已删掉群"的待发已读回执；而 `list_pending_group_reads` 只按 `peer_id` 查、
  不看群还在不在 ⇒ 这是一只**活的幽灵**（对端每次上线都会被重新捞出来投递）。
  补一条 v9→v10 清存量与否**等用户拍板**，理由写在 roadmap §12.2 那一行。
  ✅ **同日已按拍板 A 落地**（见 `[Unreleased]` 里「修复（#61）」那一节）—— 这条账不再是"留下"的了。

### Test（2026-09-26 · #59「迁移中途失败」这一格从**零判据**补成 9 条断言）

- §9「数据生命周期」点名的**部分失败**今天终于有判据：
  `migration_tests::a_failing_migration_step_rolls_back_completely_and_never_advances_the_version`。
  事务机制一直**在**（`db.rs` 五处步骤事务），但从没被证明过**真的回滚** —— "机制在"和"机制有效"是两件事。
- 钉住五面：**失败必须上抛**（且报错里要能认出注入原文，否则分不清是注入挡的还是产品别的问题）/
  **`user_version` 必须还停在升级前**（前进一步 = 下次启动**跳过这一步**，留下"版本号说升过、列却缺"的半吊子库，
  那比崩掉更坏）/ 同一步里的 `ALTER TABLE` 必须**一起回滚** / 原有 3 行消息一条不丢 /
  **拆掉注入源重放必须成功**并补上 `seq` + 回填 > 0（真实场景里"注入"就是磁盘/锁/权限抖动 ⇒ 这条就是"重启后仍然正确"）。
- ★ **注入点的选法本身是这格的产出**：上一轮我在 `outbox` 上装 `BEFORE DELETE` 触发器**是空的**
  （内联 UNIQUE 让那句清重复零行可删 ⇒ 触发器从不触发）。这次改选 **v2→v3 的 seq 回填** + `BEFORE UPDATE`
  行触发器 ⇒ "报错了"这件事**同时**就是"确有行在被写"的证据，不必再搭一层前置断言。
- ⚠️ 顺带**推翻我自己上一版写进 roadmap 的一句**："别指望 `run_migrations` 返回 Err —— v7/v8 是软失败风格"。
  实测分层才对：`db.rs:130` 是 `(step.run)(conn)?`，**步骤级一定上抛**；被吞的只是 v7/v8/v9 **闭包内部**
  逐语句的 `if let Err(e)` 容错（#60 正是被那一层藏住七个版本的）。
- ⚠️ 判"中途失败"不许走 `init()`：它会先 `execute_batch(SCHEMA)` + `ensure_post_schema_shape`
  （要在 messages 上建 `(conv_id, seq)` 索引），没有 `seq` 的老库在那一刻就先塌 ⇒ 红在错误的原因上。
- 同一处那句"插入一条数据验证 rollback 后仍存在"的**假注释**已改掉（函数体通篇没验回滚），
  并把真正的回滚判据指到新用例。§12.2 表头与 §12.9「四件必须做」的第四件同步改口；
  「数据」这一组现在**五个短语逐格具名 + 部分失败也有判据**，只剩一条待拍板的存量账（#61）。
  ✅ 那条存量账同日已按拍板 A 清掉（见本节最后那条「修复（#61）」）。

### Docs（2026-09-26 · 第五步终审表落笔，并抓到「DB 契约」今天仍不一致的实测反证）

- `docs/stability-roadmap.md` 新增 §12：把总指令 §19 的八个分组逐条写成**三态表**
  （`✅通过` 必须挂判据文件或可复跑命令；`⚠️未执行` 必须写清为什么、什么条件下才做得对；
  `🔶需真机` 只能进 Smoke 清单）。§8 阶段 E 与 §9 状态机同时指回这张表。
- ★ **本轮唯一的新事实**：`src-tauri/src/schema.sql` 与运行时真源 `db.rs` 的 `SCHEMA`
  **表名集合不等**（按 §10 那两条 grep 现算，文档版少 4 张表），且全仓没有任何代码读它
  —— ⇒ `schema.sql` 自称"与 db.rs 保持一致"是假陈述，而它同时被 `check-change-budget`
  当成 L3 敏感文件、被 `domains.data.mjs` 当成域清单成员。**"改一处必须同步改另一处"今天全靠人记得。**
  A-8 由"待决定"改记为"取证完成、等一句拍板"（两个方向：自动生成 / 直接退役，推荐退役，理由见 §12.9）。
- ⚠️ 顺带修一处**二阶漂移**：`docs/ARCHITECTURE-EXPLAINED.md` 里用来描述上面这条漂移的那行
  本身已过期（写"缺两表"，实测四张）⇒ 改为不抄数字、只挂复跑命令。
  **教训：描述漂移的句子也会漂，所以终审表里凡"N 张/N 条"都必须挂命令，不挂命令的一律重跑。**
- 结论如实：**不宣布"最终稳定版"达成**。三条不成立（DB 契约实测不一致 / 网络缺"非 LAN 出口的跨实例真跑" /
  UI 只到结构级、运行时观感零判据），其中两条等拍板、一条属环境不属代码。
- ⚠️ **同一轮内自查出两行凭记忆写的判据，已推翻重写**（口径改成"用门禁自己那份 Rust 清单现算"，不靠回忆）：
  中继与 gossip 的判据**远比记的厚**（含 token 不进探针、配置 fail-closed、无 chunk_size 拒收、
  路径逃逸、重复分片不双计；gossip 含信封签名 / TTL / fanout 排除发送者 / 同 id 只落一行 / 解密前去重）。
  ⇒ 真实缺口收窄成一句：**没有任何一次跨实例真跑走过非 LAN 路径**，而不是"这些路径没判据"。
  §12.3 / §12.4 / §12.9 三处同步改口径；gossip 那一格与 §七"三实例并发"确认是**同一块欠账**（共用同一套三实例基础设施），不是两块。

### Docs（2026-09-26 · §19「数据」那一组逐条读测试本体核对，读出一格真缺口）

- §12.2 从"有测试"升级成**具名判据**：新库（点名 12 张必需表）、旧库逐版本（`migration_legacy_..._runs_all_steps`
  / `migration_v1_to_v6_full_chain`）、不破坏旧数据（identity 两把私钥 / 消息条数与内容 / outbox / `seq` 回填 /
  删冗余索引后内联 UNIQUE 仍挡重复）、不产生幽灵状态（降级零建表 + 带类型错误 + 数表必须早于建表 +
  "只跑 SCHEMA"与"整条迁移链"形状恒等，那条红过一次真抓到一项差集）。
  ⚠️ 顺手修掉我自己两处错挂：那条"不产生幽灵状态"原先挂到并发用例上（判的是另一件事）；
  以及"全部跑在内存库"这句是错的 —— 迁移一族走**磁盘上的真实临时文件**（连 wal/shm 一起清）。
- ★ **新缺口（唯一一个"会导致用户数据丢失、又不需要谁点头"的必做项）**：
  **"迁移中途失败"这一格零判据**。步骤事务今天**已经在**（`db.rs` 五处 `unchecked_transaction()`），
  但从没有一条用例注入过一次失败 ⇒ "回滚真的发生"没被证明过。
  同一处还有一句"验证 rollback 后仍存在"的注释，而函数体写着"只验证正常跑成功"——**注释说的比代码做的多**（§13 那一类）。
  补法只加测试不改生产码：造必然失败的一步，断言 ①该步零写入 ②版本停在旧值 ③再启能续；
  **先有判据再决定要不要把建表那句也拖进同一事务**（现在它在事务之外，而"SCHEMA 单独就能修好当前库"那条恰好证明这个方向能自愈）。
