...
- **两面实测**（同一份判据，`--fault=multi-file`）：正向 `run-2026-09-26T10-34-24-478Z` 功能层 24 断言全绿
  + 契约 ✅ + `exit 0`（`trace.ids` 数得出那三张同名不同目录的单）；默认轮 `run-2026-09-26T10-34-12-978Z` 同样绿。
  反向＝只换判据的输入（id 值一字不变、只是不再登记）⇒ `run-2026-09-26T10-32-38-373Z` 功能层照旧打印全绿，
