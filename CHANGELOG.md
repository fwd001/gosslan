...
- **接法选得最便宜**：harness 的传输尺寸本来就有旋钮（`E2E_FILE_MB`），所以这一格**不需要第 9 条注入轮**
  ...
  判据 C 是**静态数 `check("…")` 调用点**、判据 D 只点名**正向 `--fault=` 轮次** ⇒ 阶梯既不加断言数
  也不占轮次登记，`check-doc-numbers` 实测仍然绿（它自己打印「1 条默认轮 + 8 条注入轮」）。
- **两档各测一个形状**（不是"越大越好"）：`--size=0.001` = 1 KB ⇒ **单片、零头即整份**；
  `--size=10` = 10 MB ⇒ **多片 + 最后一片是零头**（片数 `⌈size/片长⌉` 最容易藏 off-by-one 的位置）。
  两档实测各 16 条断言全绿，10 MB 整轮 **11.2s** ⇒ 便宜到可以长期挂在本地层。
  10 KB 与 1 KB 同为"单片"形状（LAN 片长 256 KiB）⇒ 由 1 KB 档代表，不另开一轮凑数。
- **100 MB 及以上刻意不进本地层**（一次几分钟会拖垮本地与 CI）：那一档的大文件窗口由
  「杀进程轮 100 MB」代偿覆盖，按 §十 标 SIMULATED/真机，**不写成"阶梯已全覆盖"**。
- ⚠️ **顺手记一条写注释的教训（我自己这轮先写错了一次）**：本来想在 harness 里写
  "非整数字节会让 `Buffer.alloc` 抛 RangeError"，实测**它不抛**，而是静默给一个
  1048 字节的 buffer ⇒ 于是 `bytes.length === FILE_BYTES` 变成 `1048 === 1048.576` 为假，
  报出来的红**长得像产品 bug**（"B 侧字节数与发送端一致"失败），坏的实际是档位算术。
  ⇒ `Math.round` 是这条阶梯的承重，注释按实测改写。同族教训：本仓已多次"红的是脚本不是被保护的东西"。

### Test（2026-09-26 · §七「目标目录变化」补上缺的那一半，顺手修掉三处文档自相矛盾）
- **这条格此前在生产单聊收尾路径上是零覆盖**：接收端那五条"真成功"判据（①–⑤）**没有一条走进
  rename 出口**（全部在 size / SHA 两处就被判掉），而唯一碰过 rename 失败的历史用例走的是
  `finalize_group_receive` —— 群侧另一份实现，单聊收尾改坏它照样绿。
- 新增 `network/file.rs::rotted_receive_directory_finishes_failed_never_done`：字节收完 + 哈希
  对得上，然后把下载目录整个删掉 ⇒ 判三条：**必须 Err**、台账只能 `failed` 且**不带路径**、
  不许出现成品文件。真实机制值得记一句：`FileReceiver` 持的是**已打开的句柄**，POSIX 下目录被
  unlink 后写入与 `sync_all` 照旧成功（inode 还在）⇒ **整条收尾只剩最后一次 rename 能发现
  "成品无处安放"**。这就是 §三「rename 才算完成」在文件系统故障边界上的那一格。
- **非空转已用变异证明**：把 rename 的失败吞掉（`.or_else(→Ok)`）⇒ 恰好新那条报红
  （"实得 Ok 意味着报成功而磁盘上根本没有成品"），撤掉即绿。选这个变异是因为它是最可能被
  "好心修好"的方向：为了让用户别看到失败而把改名失败当没事。
- ⚠️ **它与注入⑧不是重复**（两处文档原本都把这条格写成"未做"，现在按形状分开记）：
  ⑧ = 目录仍在但不可写（`EACCES`，两个真实进程之间）；本条 = 目录消失（`ENOENT`，生产函数）。
- **顺带修掉三处过期判据**（同一条"全部未做"的句子里，三处已被既有覆盖推翻）：
  `DB 锁竞争`已升 L2（见 `39872eb`）、`并发多文件`已由注入⑤覆盖（只覆盖"同 peer 串行 flush"
  这一时序）、"目标目录变化"按上面两半各自已做。矩阵第 12 行那句"尺寸阶梯/并发未做"与本行
  末尾自相矛盾，一并改成只保留**尺寸阶梯中段 + 错 size** 两格未做。

### Test（2026-09-26 · §十二 DB 与锁：这条耦合热点第一次有**动态**证据，另有一条判据被判定为"结构性不存在"）
- **之前只有静态**：`db 锁作用域守卫` 扫"锁内不许 emit / 不许慢活"，但**没有任何一条测试真的把两个线程
  压到同一条 `Connection` 上** ⇒ §十二 要求的 `DB lock contention test` / `长事务 test` /
  `网络线程持锁 test` 三格此前是空的（写"有守卫"等于把静态扫当动态证明，属 §十 禁止的假 PASS）。
- 新增两条（`src-tauri/src/db/clocks.rs`，模块名刻意不叫 `tests` —— 本文件被 `db.rs` 用 `include!`
  贴进去，`mod tests` 会落进 `db` 那层与既有测试模块撞名）：
  `concurrent_writers_keep_seq_contiguous_and_lose_nothing`（8 线程 × 12 条压生产写路径
  `next_clock`+`insert_message`+`touch_conversation`，判三条数据事实：**seq 连续不重号**、
  `msg_id` 唯一、一条都不丢）、
  `long_batch_transaction_does_not_wedge_plain_writers`（一边跑 500 行长事务、一边 4 个普通写入，
  判正确性 + 一条**故意给宽**的 5s 上限）。
- **非空转已用变异证明**：往锁内塞 `sleep(6s)` ⇒ 第二条恰好报红
  （`锁内不许出现秒级慢活：6.025136084s`），撤掉即绿 ⇒ 这条时间判据不是永远为真的空转。
  选它变异而不是改判据本体，是因为它正是真机 600MB 那次的形状（`finish_receive` 在锁内 fsync，
  同批并发文件被 60s 停滞判据挨个打死）。
- ⚠️ **诚实边界（写下来免得下一个人误信）**：
  ① 这两条跑在**内存库**上 ⇒ 只证明"一把锁 + 生产函数在并发下不坏"这个**形状**，
  不是磁盘 WAL/fsync 的性能量级，别拿它当"够快"的证据；
  ② 第一条**抓不到**"把取 seq 挪到锁外" —— `next_clock` 自身是一段事务、mutex 又本来就串行，
  拆开取号也不会重号（我第一版注释把这条能力写大了，已改）；要拦那一族得靠锁作用域静态守卫；
  ③ **`DB busy test` 这一格判为结构性不存在，不补**：全进程只有一条 `Connection`、外面一把
  `Mutex`，`db.rs` 里根本没有 `busy_timeout` —— 因为没有第二个连接去抢表锁。
  所以竞争只发生在 Mutex 层；把 "SQLite BUSY" 做成一条测试会是一条永远不成立的判据。
- **这条测试同时给了 §十二 那句"根据真实数据决定"一个起点**：它现在证明的是"目前这个形状没坏"，
  ⇒ 缩短锁生命周期 / `try_lock` / 后台任务隔离那一串动作，**今天都还没有被测出来的必要性**。
- 测试基线同步：macOS / Windows 两份各 +2（平台无关用例，走 `--sync-baselines` 推导，不手抄）。

### Fix（2026-09-26 · A-12：`.part` 的字节数不再被当成「我已收完」——「rename 才算完成」第一次有了活实例证明）
- **根因不是「忘了补做 rename」，是一个帧承载了两个含义**：发送端把 `FileReject.received >= size` 读作
  「对方已完整收下」并**当场收尾**（记 `done` + **删队列行**），而接收端说这句话时意思只是「我手上有这么多字节」。
  上一次收尾 `rename` 失败（目录只读 / 磁盘满 / 进程被杀在半步）时两者恰好同值 ⇒ 修前实测
  `A {status:gone, aT:done} / B=failed / .part=1048576（整份）/ final 不存在`：
  **两边各给了用户一个结论，而且互相矛盾，且队列行已删 = 再也没人重试**（2026-09-26 由注入⑧在真实双实例照出）。
- 修法打在**唯一那份判据**上（「位置判据只许有一份」是 `lib.rs` 里现成的源码守卫）：`decide_offer` 多收一个
  `size` 输入，**≥ size 的磁盘前缀一律不算进度**（按 0 处理 ⇒ 重新整份收，走真实的哈希校验与真实的 rename），
  `AlreadyHave` 仍然只认台账 `done`。三条否掉的形状与理由：**不盲改名** —— 没成品的 `.part` 内容未经校验，
  注入①早就证明它可以是随机字节，盲 rename 等于把坏文件当成品交出去；**不在握手期验 sha** —— 那是整读一份文件，
  正是 v4.22.16 刚从这条链上搬走的那个 O(体积) 开销；**不加协议字段** —— 两个含义分开之后一个帧够用，
  而协议变更要背跨版本兼容（§十四「不为漂亮而扩范围」）。
- **TDD 与反向半边**：先红 `a_full_length_part_without_rename_is_not_progress`
  （`left: ResumeFrom(1048576) / right: Accept`）⇒ 实现 ⇒ Rust 全量绿；同一格钉死反向半边
  「**小于 size 的真前缀仍然必须照常续传**」—— 把它退化成「一律重收」会打掉跨 attempt 的进度累积，
  那正是 160MB 传不完的老病根，也是注入②（真前缀续传）/ 注入③（SIGKILL 后按真实字节续完）两格活实例证明。
- 注入⑧（改口轮）由「只打印分歧」升回真断言：交叉自洽那条按 roadmap A-12 格里**抄好的原文**加回，
  另补一条「本轮 rename 恒失败 ⇒ 发送侧只能落到 failed」—— 蕴含式在前件永不成立时**永远为真**，
  不补这条就是 A-9 那格用打印救回来的同一种空转。实测 `22/22 绿`（43.3 s 内 `A=failed / attempts=5 / B=failed`，
  两侧终于一致），`-lie` 恰好 1/22 报红、退出码 1。
- ⚠️ 诚实边界：这一格证明的是**「不许在只有 `.part` 的时候声称完成」**，不是「目录恢复可写之后会自动补完」——
  协议里没有「接收端自己重试收尾」这条码，解除只读之后这一单仍停在 failed，只打印不设判据。
  接收侧「写不进去」对用户依然完全静默 → 那是 **A-10**（补提示属新增可见行为，当前阶段不做）。

### Fix 候选·发现（2026-09-26 · 注入⑧照出 A-12：`AlreadyHave` 短路绕过「rename 才算完成」⇒ 发送侧 done / 接收侧 failed）
- **新加了一格故障注入，它直接把一个 A 类缺陷照了出来**（未修，先钉判据 + 登记 roadmap **A-12**，等用户拍板 —— 动的是文件收尾判定）。
- 注入形状是⑤那条注释里点名"要另开一条"的 **A-5**：先给接收端预置**真前缀** `.part`（⇒ offer 不被拒、字节照流），
  再把接收目录改成只读。只读目录**不影响**往已存在的文件里写，只影响 **create / rename / unlink** ⇒ 唯一会塌的动作就是收尾那次 `rename`。
  顺序即确定性，不靠撞窗口（沿用冻结轮"先冻 → 再入队"的招）。
- ★ **实测（13.6 s 内定论）**：`A 终态 {status:gone, aT:done} · B=failed · .part=1048576（整份）· final 不存在`。
  两侧日志互为证据：B 两次报"接收端已有 65536 / 1048576 字节，要求从此续发"，
  A 第二次看到 `received >= size` 就走了「接收端已完整收下该文件，直接完成」。
- **为什么这比"传失败"更糟**：接收侧"我已收到多少"是按 **`.part` 现有字节数**答的，而 `.part` 存在 ≠ 那次 rename 成功过。
  于是发送侧宣布完成并**删掉队列行**（= 再没有任何东西会去补它），接收侧记 `failed`，
  文件整份躺在一个改不了名的 `.part` 里 —— **两边各给用户一个结论，而且互相矛盾**。
  「rename 才算完成」这条"物理定律"第一次被活实例证伪（此前只有 Rust 单测在函数级守它）。
- 判据形状刻意**不预设"产品必须失败"**（那是替产品做决定）：4 条断言只钉"每一侧自己不撒谎"
  （`.part` 真被续写 / A 侧终态明确 / attempts 有界 / 接收侧台账不假 done），
  **交叉自洽那条**（"发送侧宣布完成 ⇒ 接收侧必须有整份正确的 final 文件"）今天不成立 ⇒ 按 A-11 的先例
  **只用打印钉住分歧、不写成断言**，原文抄在 roadmap A-12 那一格里，修完再加回来。
- ★ 为什么不能"先把红判据挂着，等修完"：这套门禁**没有"容忍已知红"这一档**（`verify.mjs` 里没有 soft/allowFail，
  是刻意的），而 `check-doc-numbers` 判据 D 又要求"正向轮次必须被 local 层点名"——
  两条一起把"加一条注入然后让它红在角落"这条路堵死了。**结果是被守卫推着做的**：不是我想到了要接进门禁。
  （顺带被抓住的第二处：`check-doc-numbers` 的轮次标签是一份**手写正则**，新轮次不进去 ⇒ 它报"声明消失了"。）
- ★ 顺带修掉我自己写判据时的一处误判：第一版把「A 侧 `file_outbox` 行被收尾删除」当成"还没到终态"继续等 ⇒
  跑满 180 s 超时，把**成功**读成了**卡住**。行没了就是终态（J2 那条判据早写着），必须立刻收。

### Test（2026-09-26 · INV-P18「私钥不跨 UI 边界」从一句 prose 变成两条静态判据）
- **根因**：上一轮把 26 条不变量逐条绑钩子时，P18 是唯一拿不出对象的一条 —— 它今天成立只是因为
  "恰好没有任何命令返回密钥字段"，而**破掉它只需要给 `Identity` 补一行 `#[derive(Serialize)]`**（或让某条命令
  `Ok(密钥字节编码)`），界面完全无症状、CI 一声不响。写 `NONE` 是诚实的，但不是收口。
- 新增 `scripts/check-key-boundary.mjs`（已进 `npm run verify` 快速层），两条判据 + **5 条夹具自证**（含 2 条"不许误伤"反例）：
  **A** 任何带 `#[derive(…Serialize…)]` 的类型，字段名命中 `secret|private_key|signing|seed|keypair|sk` 或类型是
  `StaticSecret|EphemeralSecret|SharedSecret|SigningKey|KeyPair|Zeroizing` ⇒ 红；
  **B** 任何 `#[tauri::command]` 的命令体，在**出口位置**（`return` / `Ok(` / `emit` / `.to(`）触碰密钥标识符 ⇒ 红，
  并逐条与 `generate_handler!` 注册表对账 ⇒ **有已注册命令没被扫到也算红**（挡住"判据悄悄漏掉一条命令"这种假绿）。
- 先量再写规则：实测全项目私钥载体只有 `Identity.x25519_secret`/`Identity.ed25519_signing`（`crypto.rs`）与
  `EphKeypair.secret`（`transport/relay_seal.rs`）三处，且**都没有 derive** ⇒ 判据 A 现在 66 个可序列化类型零命中，
  判据 B 覆盖 133 条命令体（注册表 129 条，差额是 `#[tauri::command]` 定义数 > 注册数，逐条对上）。
- ★ **两条判据都是被自己的夹具逼出来的**：判据 B 第一版按"命令体里出现密钥标识符"判 ⇒ `send_message`
  /`distribute_group_key` 这类"命令内部做 ECDH、只返回密文"的正常写法全红。收窄到"只在出口位置判"之后，
  **收窄本身也要机器证明** ⇒ 加了那条反例夹具（内部用私钥 ⇒ 必须绿），否则下一次放宽没人拦得住。
- ★ 夹具还抓到一处我自己写的 bug：`blockFrom()` 的"单行早返回"提前 return 本该只在函数签名那一行生效，
  没判 `i === start` ⇒ 函数体内第一个无括号 `let …;` 就把命令体截断成一行，判据 B 静默抓到 0 处、
  自证第一次跑就把它判成"这条判据抓不住"。**"看起来在扫"和"真扫到了"的差别只有夹具能分辨。**
- ⚠️ **诚实边界（不许读成"私钥泄漏已被彻底堵住"）**：两条判据都是**按形状扫**，挡得住"补一行 derive"和
  "命令直接 `Ok(密钥)`"，挡不住"先 `let k = sec.to_bytes()` 再 `Ok(k)`"这种**两步搬运**。
  根治要改生产码：把 `Identity` 的两个密钥字段改私有、只暴露 `sign()` / `dh()`，让私钥**按构造不可导出**。
  那属核心码重构，另立任务，不混在这次里（§十八）。
- **非空转证明已登记**：`verify-guards.py` 新 Case（给 `crypto.rs` 的 `Identity` 注入一行 `#[derive(Clone, Debug, serde::Serialize)]`
  ⇒ FAIL、撤掉 ⇒ PASS），现场跑出 ✅；护栏非空转用例计数由该步自己打印。
- 顺带把上一节留的 P18 `NONE` 换成真钩子 ⇒ `check-invariant-hooks.mjs` 现在报 **26 条全部绑定 / 0 条登记为无自动化**。

### Test（2026-09-26 · 「物理定律」从 prose 变成锁：26 条不变量逐条绑定到当场能解析的钩子）
- **roadmap 里的 R3/B-1（原判"等 harness 再补 20 条测试"）收口，但收法与原判不同，而且原判定是错的**：
  先补测试会白做 —— 没有任何东西保证"文档里那句话指的就是这条测试"，测试改名/删除时文档照样好看。
  真正的缺口是**绑定本身没有判据**（26 条里只有 6 条写了具名对象，其余全是「验证」段里的一段伪码）。
- 新增 `scripts/check-invariant-hooks.mjs`（已进 `npm run verify` 快速层）：每条 `### INV-Pxx` 必须有一行
  `- 钩子：`，且每个名字**当场能解析** —— `a::b::c` 必须在本平台 Rust 基线里；`scripts/x.mjs` 必须存在
  **且已接进门禁**（verify.mjs / package.json 搜不到这个名字即红，挡"写完没接线的绿"）；`guards:` 片段必须命中
  `verify-guards.py` 的真实 Case 名；`e2e:` / `smoke:` 片段必须命中多实例 harness 判据 / 平台 Smoke 清单；
  `x.test.ts#标题片段` 必须真在那份前端测试里。拿不出名字就只能写 `NONE —— 理由`，**且每轮都在末尾点名打印**，
  不让"没有测试"退化成新的 prose 债（总指令§十）。
- 逐条绑完：**25 条具名 / 1 条（P18）显式登记为无自动化**，形态含 Rust 用例、护栏脚本、非空转 Case、
  多实例注入判据、前端契约测试。**非空转证明已登记**（`verify-guards.py` 新 Case：把 INV-P01 的钩子换成
  一个不存在的用例名 ⇒ FAIL、恢复 ⇒ PASS，已跑出 ✅），护栏非空转用例计数由该步自己打印。
- ★ 过程中被这条新判据自己抓到的第一处错：我在两条里把 `unknown_wire_frame_is_tolerated_after_auth`
  写成 `network::transport::tests::…`，真名字挂在 crate 根的 `tests::…` 下 ⇒ **首次运行即红**。
  这正是这条守卫的形状：名字写错=绑定不存在，而写错的概率在"手工抄名字"这件事里几乎是必然的。
- ⚠️ 诚实边界（不要当成"不变量全部有测试了"）：本判据只保证**绑定不失效**，不保证**行为已被穷举**。
  两处已知的半空：① P03 的子句"解密失败不 Ack"今天没有专属用例（现有钩子覆盖"落库才算 ACK"与"ACK 验归属"）；
  ② P18"私钥不跨 UI 边界"只靠"没有任何命令返回它"这个事实，没有判据钉住。两格合记为 **B-1a（A 类，安全优先）**。
  （② 已在同日稍后由 `check-key-boundary.mjs` 封住，见上一节；B-1a 只剩 P03 那一格。）
...
- ⚠️ **诚实边界**：这 679 条是"从 macOS 观测名单 + 源码门控**推**出来的"，不是 Windows 上观测过的。
  ...
  另：该判据的注入夹具已登记进 `verify-guards.py`（Case「平台基线之间的差额说不出平台理由」，故意从 Windows 基线
  删一条 ⇒ FAIL、恢复 ⇒ PASS），护栏用例 185→**187**。它跑在**不需要 cargo** 的那条子集上 —— 判据本身也因此
  从 `checkRust` 里挪到最外层：留在里面就会让"Rust 编不过"顺手关掉跨平台这只眼。
