...
蓝牙用于近场加入。节点既互为中继也可以直连；**没有强制的官方后台** —— 没有账号、没有注册、
没有托管服务、没有强制升级，每台设备都是对等的一份。
跨 CGNAT 时可以由**用户自己**跑一台哑管道中继补一条链路（ADR-0020）：它不解析载荷、
不落盘、不持有任何数据，且只有用户在设置里填了地址才参与。
...
* 账号体系、服务端注册、**官方自己运营任何后台服务**。
  2026-09-21 由 ADR-0020 精确化：禁的是"需要官方有个后台"的东西（账号、注册、托管、配额），
  **不是**"用户自己跑一台机器"。跨网段的可选自托管哑管道中继属于已批准范围
  （服务器在独立仓库 <https://github.com/fwd001/gosslan-relay-server>，客户端侧是 `transport/relay_seal.rs`，见 ADR-0020）：
  它无账号、不注册、不解析载荷、
  不落盘，且只在用户自己填写服务器地址时才生效。做这类"用户自带服务器"的扩展时，
  必须保持：官方不部署、不持有数据、关掉开关后行为与今天逐字节一致。
